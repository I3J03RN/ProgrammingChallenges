...
void solve() {
    ...
    F0R (i, n) {
        cin >> xs[i] >> ys[i] >> ns[i] >> ms[i];
    }
    adj.resize(1 + 2 * n);
    int source = 2 * n; 
    int allPs = 0;
    F0R (i, n) {
        addEdge(source, i * 2, ns[i]);
        allPs += ns[i];
        addEdge(i * 2, i * 2 + 1, ms[i]);
    }
    F0R (i, n) {
        FOR (j, i + 1, n) {
            if (hypot(xs[i] - xs[j], ys[i] - ys[j]) <= d) {
    ...
    vi safe;
    F0R (i, n) {
        for (auto& e : edges) e.mflow = 0;
        if (allPs == maxflow(source, i * 2)) {
            safe.pb(i);
        }
    }
    F0R (i, SZ(safe)) {
