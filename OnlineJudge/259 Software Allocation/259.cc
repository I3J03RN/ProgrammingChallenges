...
int main() {
    ...
    while (cin) {
        ...
        while (1) {
            ...
            runOn.eb();
            for (int i = 3; in[i] != ';'; ++i) {
                runOn.back().pb(in[i] - '0');
            }
        }
        dout << dvar(jobs) << endl << dvar(runOn) << endl;
        adj.resize(12 + SZ(jobs));
        int s = 10, t = 11;
        F0R (i, 10) {
            addEdge(s, i, 1);
        }
        F0R (i, SZ(jobs)) {
            addEdge(12 + i, t, jobs[i].se);
            for (int m : runOn[i]) {
                addEdge(m, 12 + i, 1);
            }
        }
        if (jobCnt == maxflow(s, t)) {
            string out(10, '_');
            for (auto& e : edges) {
                if (e.mfrom < s && e.mto > t  && e.mflow == 1) {
                    out[e.mfrom] = jobs[e.mto - 12].fi;
                }
            }
            cout << out << endl;
        } else {
