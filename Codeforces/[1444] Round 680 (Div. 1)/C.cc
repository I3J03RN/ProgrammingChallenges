...
int main() {
  ...
  vi cnt(k);
  vector<bool> bs(k);
  auto dfs = y_combinator([&](auto self, int v, int lc) -> bool {
    col[v] = lc ^ 1;
    for (int u : adj[v]) {
      if (~col[u]) {
        if (col[u] == col[v]) return false;
      } else {
        if (not self(u, col[v])) return false;
      }
    }
    return true;
  });
  F0R (i, n) if (col[i] == -1 && not bs[cs[i]]) {
    bs[cs[i]] = !dfs(i, cnt[cs[i]]);
    cnt[cs[i]] += 2;
  }
  k -= count(ALL(bs), true) + 1;
  ll gd = k * (k + 1) / 2;
  sort(ALL(es), [&](const auto& a, const auto& b) {
    return tie(cs[a.fi], cs[a.se]) < tie(cs[b.fi], cs[b.se]);
  });
  int l = -1, r = -1;
  map<ii, vii> adj2;
  map<ii, bool> col2;
  auto dfs2 = y_combinator([&](auto self, ii v, bool lc) -> bool {
    col2[v] = not lc;
    for (auto u : adj2[v]) {
      if (col2.count(u)) {
        if (col2[v] == col2[u]) return false;
      } else {
        if (not self(u, col2[v])) return false;
      }
    }
    return true;
  });
  auto test = [&]{
    for (const auto& [i, v] : adj2) {
      if (!col2.count(i) && !dfs2(i, true)) {
        --gd;
        break;
      }
    }
  ...
  F0R (i, SZ(es)) {
    if (bs[cs[es[i].fi]] || bs[cs[es[i].se]]) continue;
    if (cs[es[i].fi] != l || cs[es[i].se] != r) {
      l = cs[es[i].fi]; r = cs[es[i].se];
      test();
    }
    ii a = mp(cs[es[i].fi], col[es[i].fi]);
    ii b = mp(cs[es[i].se], col[es[i].se]);
    for (auto j : {a, b}) if (adj2[j].empty()) {
      ii o = j ^ mp(0, 1);
      adj2[j].pb(o);
      adj2[o].pb(j);
    }
