...
void solve() {
  ll x, y; cin >> x >> y;
  vector<ll> cs(6); cin >> cs;
  ll best = numeric_limits<ll>::max();
  auto calcc = [&](ll a, ll b) -> ll {
    if (a == 0 && b == 0) return 0;
    if (a > 0 && b > 0) return cs[0] * a;
    if (a == 0 && b > 0) return cs[1] * b;
    if (a < 0 && b == 0) return cs[2] * -a;
    if (a < 0 && b < 0) return cs[3] * -a;
    if (a == 0 && b < 0) return cs[4] * -b;
    return cs[5] * a;
  };
  auto calc = [&](ll a, ll b) -> ll {
    ll res = calcc(a, b);
    dout << dvar(a, b, res) << endl;
    return res;
  };
  array<ll, 6> xd{1, 0, -1, -1, 0, 1}, yd{1, 1, 0, -1, -1, 0};
  F0R (i, 6) {
    if (xd[i]) {
      ll n = -x / xd[i];  
      dout << "a" dvar(i, n) << endl;
      if (n >= 0) ckmin(best, cs[(i + 3) % 6] * n + calc(x + n * xd[i], y + n * yd[i]));
    }
    if (yd[i]) {
      ll n = -y / yd[i];
      dout << "b" dvar(i, n) << endl;
      if (n >= 0) ckmin(best, cs[(i + 3) % 6] * n + calc(x + n * xd[i], y + n * yd[i]));
    }
    if (yd[i] != xd[i]) {
      if (xd[i]) {
        ll n = (y - x) / xd[i];
        dout << "c" dvar(i, n) << endl;
        if (n >= 0) ckmin(best, cs[(i + 3) % 6] * n + calc(x + n * xd[i], y + n * yd[i]));
      } else {
        ll n = (x - y) / yd[i];
        dout << "d" dvar(i, n) << endl;
        if (n >= 0) ckmin(best, cs[(i + 3) % 6] * n + calc(x + n * xd[i], y + n * yd[i]));
      }
    }
  }
