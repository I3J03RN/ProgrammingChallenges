...
template<typename T, typename F>
ST<T, F> make_st(int sz, T u, F m) {
  return ST<T, F>(sz, u, m);
}
...
int main() {
  ...
  ST st{n + 2, numeric_limits<int>::max(), [](int a, int b) { return min(a, b); }};
  ...
  st.build();
  vvi ps(n + 2);
  for (auto& v : ps) v.pb(-1);
  F0R (i, n) ps[as[i]].pb(i);
  for (auto& v : ps) v.pb(n);
  ...
  vi idx(SZ(qs));
  iota(ALL(idx), 0);
  sort(ALL(idx), [&](int a, int b) { return qs[a] < qs[b]; });
  vi ans(SZ(qs));
  ...
  FOR (i, 1, n + 2) {
    int l = ps[i][0];
    FOR (j, 1, SZ(ps[i])) {
      if (l < ans[qidx] && l + 1 != ps[i][j] && mex == i) ++mex;
      ++qidx;
      l = ps[i][j];
    }
  }
