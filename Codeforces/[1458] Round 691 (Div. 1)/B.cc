...
int main() {
  ...
  vii is(n); cin >> is;
  int sm = 0;
  for (auto [a, _] : is) sm += a;
  vector<vector<double>> dp(n, vector<double>(sm + 1, -1e18));
  sm = 0;
  dp[0][0] = 0;
  // cout << pp(dp, "\n", " ") << endl;
  F0R (i, n) {
    ...
    for (int k = n - 1; ~k; --k) {
      for (int c = sm; ~c; --c) {
        if (c - a >= 0) ckmax(dp[k][c], dp[k][c - a] + b);
        if (k) ckmax(dp[k][c], dp[k - 1][c] + b / 2.0);
      }
    }
    // cout << pp(dp, "\n", " ") << endl;
  }
  for (int i = n - 1; ~i; --i) {
    double best = 0;
    F0R (c, sm + 1) ckmax(best, min((double)c, dp[i][c]));
