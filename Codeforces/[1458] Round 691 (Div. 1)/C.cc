...
using F = linear<int, 3>;

void solve() {
  int n, m; cin >> n >> m;
  mod::p = n; 
  vvi mat(n, vi(n)); string s; cin >> mat >> s;
  F::M row{{1, 0, 0}, {0, 0, 1}, {0, 1, 0}};
  F::M col{{0, 0, 1}, {0, 1, 0}, {1, 0, 0}};
  F::V r{0, 1, 0}, l{0, -1, 0}, d{1, 0, 0}, u{-1, 0, 0};
  F f{};
  for (char c : s) {
    if (c == 'R') {
      f.combine(r); 
  ...
  F0R (rr, n) {
    F0R (cc, n) {
      auto v = f.calc({rr, cc, mat[rr][cc] - 1});
      res[conv(v(0))][conv(v(1))] = conv(v(2)) + 1;
    }
  }
  cout << pp(res, "\n", " ") << endl;
}
