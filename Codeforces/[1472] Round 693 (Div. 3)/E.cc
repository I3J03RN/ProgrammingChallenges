...
void solve() {
  int n; cin >> n;
  vi hs(n), ws(n);
  F0R (i, n) cin >> hs[i] >> ws[i];
  vi hss(n), wss(n); iota(ALL(hss), 0); iota(ALL(wss), 0);
  sort(ALL(hss), [&](int a, int b) { return hs[a] < hs[b]; });
  sort(ALL(wss), [&](int a, int b) { return ws[a] < ws[b]; });
  vi phs(n), pws(n);
  F0R (i, n) {
    phs[hss[i]] = i;
    pws[wss[i]] = i;
  }
  ST hst(n, -1, [&](int a, int b) { if (b == -1) return a; if (a == -1) return b; return (min(mp(ws[a], a), mp(ws[b], b))).se; });
  ST wst(n, -1, [&](int a, int b) { if (b == -1) return a; if (a == -1) return b; return (min(mp(hs[a], a), mp(hs[b], b))).se; });
  F0R (i, n) {
    hst.data[n + i] = hss[i];
    wst.data[n + i] = wss[i];
  }
  hst.build(); wst.build();
  dout << dvar(hs, hss, phs) << endl << dvar(ws, wss, pws) << endl;
  F0R (i, n) {
    auto it = lower_bound(ALL(hss), i, [&](int a, int b) { return hs[a] < hs[b]; });
    if (it != hss.begin()) {
      int e = it - hss.begin();
      int w = hst.query(0, e);
      dout << dvary(e, w) << endl;
    ...
    auto itt = lower_bound(ALL(wss), i, [&](int a, int b) { return ws[a] < hs[b]; });
    if (itt != wss.begin()) {
      int e = itt - wss.begin();
      dout << dvarc(e) << endl;
      if (pws[i] < e) {
        if (pws[i] != 0) {
          int h = wst.query(0, pws[i]);
          if (hs[h] < ws[i]) {
            cout << h + 1 << ' ';
            continue;
          }
