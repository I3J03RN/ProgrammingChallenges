...
struct tensor {
  vector<T> data;
  array<size_t, dim> ds;
  tensor(initializer_list<int> dims, T v = T())
    : data(accumulate(ALL(dims), (size_t)1, multiplies<size_t>()), v) {
...
void solve() {
  int n, m, k; cin >> n >> m >> k;
  vvi as(n, vi(m)); cin >> as;
  if (m == 1) {
    cout << 0 << endl;
    return;
  }
  tensor<int, 4> dp{{n, m, m / 2 + 1, k}, numeric_limits<int>::min()}; 
  F0R (r, n) {
    if (r) {
      F0R (j, k) {
        F0R (i, m / 2 + 1) {
          ckmax(dp[{r, 0, 0, j}], dp[{r - 1, m - 1, i, j}]);
        }
      }
    } else {
      dp[{r, 0, 0, 0}] = 0;
    ...
    F0R (j, k) {
      dp[{r, 0, 1, (j + as[r][0]) % k}] = dp[{r, 0, 0, j}] + as[r][0]; 
    }
    FOR (c, 1, m) {
      F0R (j, k) {
        ckmax(dp[{r, c, 0, j}], dp[{r, c - 1, 0, j}]);
        int v = (k + j - as[r][c] % k) % k;
        FOR (i, 1, m / 2 + 1) {
          dp[{r, c, i, j}] = max(dp[{r, c - 1, i - 1, v}] + as[r][c], dp[{r, c - 1, i, j}]);
        }
      }
    }
  }
