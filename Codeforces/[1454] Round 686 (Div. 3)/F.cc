...
array<int, 3> dl{0, -1, 0}, dr{0, 0, 1};

void solve() {
  int n; cin >> n;
  vi as(n); cin >> as;
  SPT mx(as, [](int a, int b) { return max(a, b); });
  SPT mn(as, [](int a, int b) { return min(a, b); });
  set<int> s(ALL(as));
  for (int i : s) {
    int lb = -1, hb = n - 1;
    while (lb < hb) {
    ...
    int r = lb;
    dout << dvar(i, l, r) << endl;
    F0R (j, 3) {
      int x = l + 1 + dl[j];
      int y = r + dr[j];
      if (x <= 0 || y >= n || y <= x) continue;
      if (mx.query(0, x) == i && mn.query(x, y) == i && mx.query(y, n) == i) {
        cout << "YES" << endl;
        cout << x << ' ' << y - x << ' ' << n - y << endl;
        return;
      }
  ...
  int m = *s.rbegin();
  FOR (i, 1, n - 1) {
    if (as[i] == m && mx.query(0, i) == m && mx.query(i + 1, n) == m) {
      cout << "YES" << endl;
      cout << i << ' ' << 1 << ' ' << n - 1 - i << endl;
      return;
    }
