  ...
  SPT(vi& A) {
    ...
    for (int j = 1; (1 << j) <= SZ(A); ++j)
      for (int i = 0; i + (1 << j) <= SZ(A); ++i) {
        ...
        if (ckmax(spT[i][j], spT[i][j - 1])) idx[i][j] = idx[i][j - 1];
    }
  }
  ii rmq(int l, int r) {
    int k = 31 - __builtin_clz(r - l + 1);
    return max(mp(spT[l][k], idx[l][k]),
...
void solve() {
  ...
  while (q--) {
    ...
    int l = max(0, k - 1 - (s - 1)), r = min(k - 1, n - s);
    while (l < r) {
      int mid = (l + r + 1) / 2;
      int ri = s - 2 + mid, li = s - 1 - (k - mid);
      if (spt.rmq(s - 1, ri) < spt.rmq(li, s - 2)) {
        l = mid;
      } else {
        r = mid - 1;
      }
    ...
    if (spt.rmq(s - 1, s - 2 + l) < spt.rmq(s - 1 - (k - 1 - l), s - 2)) {
      cout << s - (k - 1 - l) << ' ';
    } else {
      cout << s + l << ' ';
    }
