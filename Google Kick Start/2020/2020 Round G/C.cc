...
void solve() {
  int w, n; cin >> w >> n;
  vector<ll> xs(w); cin >> xs;
  ll best = numeric_limits<ll>::max();
  sort(ALL(xs));
  vector<ll> vs; vs.reserve(w * 3);
  for (ll i = -n; i <= n; i += n) for (auto x : xs) vs.pb(x + i);
  ll val = 0;
  map<ll, ll> cnt;
  F0R (i, w) {
    val += vs[w] - vs[i];
  ...
  int last = 0;
  for (int i = w; i < 2 * w; ++i) {
    while (last < 2 * w && vs[i] - vs[last] > vs[last + w] - vs[i]) {
      --cnt[vs[last]]; ++cnt[vs[last + w]];
      val -= vs[i] - vs[last];
      val += vs[last + w] - vs[i]; 
      if (vs[last + w] > vs[i]) --leq;
      ++last;
    }
    ckmin(best, val);
    if (vs[i + 1] > vs[i]) {
      val += leq * (vs[i + 1] - vs[i]);
      val -= (w - leq) * (vs[i + 1] - vs[i]);
      leq += cnt[vs[i + 1]];
    }
  }
