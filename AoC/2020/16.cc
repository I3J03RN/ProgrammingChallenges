...
int main() {
  ...
  int idx = 0;
  map<string, vi> fields;
  vi my;
  vvi other;
  input([&](auto& in) {
    if (idx < 20) {
      string s; vi v(4); in >> s >> v;
      fields[s] = v;
    } else if (idx == 20) {
      int x; 
      while (in >> x) my.pb(x);
  ...
  for (auto& v : other) {
    ...
    for (auto i : v) {
      bool ok = false;
      for (auto [k, vv] : fields) {
        ok |= (vv[0] <= i && i <= vv[1]) || (vv[2] <= i && i <= vv[3]);
      }
      if (not ok) {
        res += i;
        good = false;
  ...
  for (auto& v : valid) {
    F0R (j, SZ(v)) {
      int i = v[j];
      for (auto [k, vv] : fields) {
        if (!(vv[0] <= i && i <= vv[1]) && !(vv[2] <= i && i <= vv[3])) {
          ca[j].erase(k);
        }
      }
    }
  ...
  F0R (_, 20) {
    int n = -1;
    F0R (i, 20) {
      if (not used[i] && SZ(ca[i]) == 1) {
        used[i] = true;
        n = i;
      }
    }
    F0R (i, 20) {
      if (i != n) ca[i].erase(*ca[n].begin());
    }
  }
  string p = "departure";
  ll ans = 1;
  F0R (i, 20) {
    if (ca[i].begin()->substr(0, SZ(p)) == p) ans *= my[i];
  }
  cout << ans << endl;
