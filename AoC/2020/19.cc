...
int main() {
  ...
  input([&](auto& in) {
    if (f) {
      int n; in >> n;
      cout << n << endl;
      while (SZ(rules) <= n) rules.eb(0, vvi(1));
      if (n == -1) {
        f = not f; 
      } else {
        string s;
  ...
  F0R (star, 2) {
    if (star) {
      rules[8].se.pb({42, 8});
      rules[11].se.pb({42, SZ(rules)});
      rules.eb(0, vvi{{11, 31}});
    }
    res.pb(0);
    F0R (_, SZ(ss)) {
      const string& s = ss[_];
      ...
      F0R (i, n) {
        F0R (j, SZ(rules)) {
          if (s[i] == rules[j].fi) dp[i][1].insert(j);
        }
      }
      FOR (l, 2, n + 1) {
        F0R (i, n + 2 - l) {
          for (int j : ones) {
            for (auto& v : rules[j].se) {
              if (SZ(v) == 1 && dp[i][l - 1].count(v[0])) {
                dp[i][l - 1].insert(j);
              }
            }
          }
        ...
        F0R (i, n + 1 - l) {
          FOR (j, i + 1, i + l) {
            F0R (k, SZ(rules)) {
              if (rules[k].fi) continue;
              for (auto& v : rules[k].se) {
                if (SZ(v) != 2) continue;
                if (dp[i][j - i].count(v[0]) && dp[j][l - j + i].count(v[1])) {
                  dp[i][l].insert(k);
                }
              }
            }
      ...
      res.back() += dp[0][n].count(0);
      cout << string(star + 1, '*') << string(2 - star, ' ') << _ + 1 << '/' << SZ(ss) << endl;
    }
  }
  F0R (star, 2) {
    cout << string(star + 1, '*') << ": " << res[star] << endl;
  }
