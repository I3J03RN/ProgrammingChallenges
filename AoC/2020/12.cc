...
void solve1(const vector<pair<char, int>>& ins) {
  int r = 0, c = 0;
  array<char, 4> dirs{'E', 'S', 'W', 'N'}; int idx = 0;
  auto go = [&](char d, int n) {
    if (d == 'N') {
      r -= n;
    } else if (d == 'S') {
      r += n;
...
void solve2(const vector<pair<char, int>>& ins) {
  int r = 0, c = 0;
  int wr = -1, wc = 10;
  for (auto [d, n] : ins) {
    if (d == 'N') {
...
int main() {
  ...
  input([&](auto& in) {
    char c; int n; in >> c >> n; ins.eb(c, n);
  });
