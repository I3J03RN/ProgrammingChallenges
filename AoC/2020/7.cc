...
int main() {
  ...
  unordered_set<string> seen{"shinygold"};
  ...
  for (bool change = true; change;) {
    change = false;
    for (auto& [k, v] : adj) {
      if (any_of(ALL(v), [&](const auto& s) { return seen.count(s.fi); })) {
        change |= seen.insert(k).se;
      }
    }
  }
  unordered_map<string, ll> dp;
  auto solve = y_combinator([&](auto self, const string& s) -> ll {
    if (dp.count(s)) return dp[s];
    ll sum = 0;
    for (const auto& [ss, n] : adj[s]) {
      sum += n * (1 + self(ss));
    }
    return dp[s] = sum;
  });
  cout << SZ(seen) - 1 << endl;
  cout << solve("shinygold") << endl;
