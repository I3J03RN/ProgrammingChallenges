...
int play(deque<int> one, deque<int> two, bool star = false) {
  static int lvl = 0; ++lvl;
  set<deque<int>> so, st;
  if (lvl > 1) {
    int N = SZ(one) + SZ(two);
    int mxo = *max_element(ALL(one)), mxt = *max_element(ALL(two));
    if (mxo > max(N - 2, mxt)) {
      return --lvl, 1;
    }
  }
  while (SZ(one) && SZ(two)) {
    int win = 0;
    if (so.count(one) || st.count(two)) {
      win = 1;
    }
    so += one;
    st += two;
    int o = one.front(); one.pop_front();
    int t = two.front(); two.pop_front();
    if (not win) {
      if (star && o <= SZ(one) && t <= SZ(two)) {
        win = 1 + (play(deque<int>(one.begin(), one.begin() + o), deque<int>(two.begin(), two.begin() + t)) < 0);
      } else {
        win = 1 + (t > o);
      }
    }
    if (win == 2) swap(o, t);
    auto& q = win == 1 ? one : two; q.insert(q.end(), {o, t});
  }
  int mu = 1, sc = 0;
  if (one.empty()) {
    swap(one, two);
  ...
  for (ll m = 1; not one.empty(); ++m) {
    sc += m * one.back(); one.pop_back();
  }
  return --lvl, sc * mu;
}
...
int main() {
  ...
  cout << play(one, two) << endl;
  cout << abs(play(one, two, true)) << endl;
