...
string solve(string s, int N, int I, bool star) {
  auto nxt = [&](int i) {
    if (i == 1) return N;
    return i - 1;
  };
  vi vs(N + 1); iota(ALL(vs), 0);
  F0R (i, SZ(s)) vs[i + 1] = s[i] - '0';
  vii ls(N + 1);
  FOR (i, 2, N) {
    ls[vs[i]] = mp(vs[i - 1], vs[i + 1]);
  }
  ls[vs[1]] = mp(vs[N], vs[2]);
  ls[vs[N]] = mp(vs[N - 1], vs[1]);
  int cur = vs[1];
  F0R (_, I) {
    int b = cur, e = ls[cur].se;
    vi mv;
    for (int i = 0; i < 3; ++i, e = ls[e].se) mv.pb(e);
    ls[b].se = e;
    ls[e].fi = b;
    swap(cur, e);
    b = nxt(e);
    while (find(ALL(mv), b) != mv.end()) b = nxt(b);
    e = ls[b].se;
    ls[b].se = mv[0];
    ls[mv[0]].fi = b;
    ls[mv[2]].se = e;
  ...
  if (star) {
    return to_string((ll)ls[1].se * ls[ls[1].se].se);
  } else {
    string out;
    for (int it = ls[1].se; it != 1; it = ls[it].se) {
      out += to_string(it);
    }
    return out;
  }
...
int main() {
  ...
  string s; cin >> s;
  cout << solve(s, SZ(s), 100, false) << endl;
  cout << solve(s, 1e6, 1e7, true) << endl;
