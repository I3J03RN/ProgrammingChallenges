...
ll eval(const string& s, int& pos, ll last = -1, char op = -1) {
  if (pos == SZ(s)) return last;
  if (s[pos] == '(') {
    ++pos;
    ll v = eval(s, pos);
    if (op == -1) {
      return eval(s, pos, v, -1);
    } else if (op == '+') {
      return eval(s, pos, last + v, -1);
    } else {
      return eval(s, pos, last * v, -1);
    }
  } else if (s[pos] == ')') {
    ...
    ++pos;
    return eval(s, pos, last, s[pos - 1]);
  }
}
