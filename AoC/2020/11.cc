...
array<int, 8> dr{1, 1, 1, -1, -1, -1, 0, 0}, dc{0, 1, -1, 0, 1, -1, 1, -1};

int main() {
  ios_base::sync_with_stdio(0);
  cin.tie(0);

  vector<string> sss = input([&](auto&) {});
  int R = SZ(sss), C = SZ(sss[0]);
  auto ok = [&](int r, int c) {
    return 0 <= min(c, r) && r < R && c < C;
  };
  F0R (_, 2) {
    vector<string> ss = sss;
    auto f = [&](int r, int c, int i) -> bool {
      for (r += dr[i], c += dc[i]; _ && ok(r, c) && ss[r][c] == '.'; r += dr[i], c += dc[i]);
      return ok(r, c) && ss[r][c] == '#';
    };
    while (true) {
      vector<string> ns = ss;
      F0R (r, SZ(ns)) {
