...
vector<bool> used;
vvi placed;
array<int, 4> rd{0, 1, 0, -1}, cd{1, 0, -1, 0};
int N = 0, L = 0;
bool gtg(int r, int c) { return min(c, r) >= 0 && max(r, c) < L; }

vector<vector<set<int>>> adj;
bool place(int v, int r = 0, int c = 0, int midx = 0) {
  F0R (i, 4) {
    int rr = r + rd[i], cc = c + cd[i];
    if (gtg(rr, cc) && ~placed[rr][cc] && adj[(i + 2) % 4][placed[rr][cc]].count(v) + adj[i][v].count(placed[rr][cc]) != 2) {
      return false;
    }
  }
  if (placed[r][c] != -1) return true;

  placed[r][c] = v;
  used[v / 12] = true;
  if (not gtg(r + rd[midx], c + cd[midx]) || ~placed[r + rd[midx]][c + cd[midx]]) midx = ++midx % 4;
  int nr = r + rd[midx], nc = c + cd[midx];
  if (~placed[nr][nc] && place(placed[nr][nc], nr, nc)) return true;
  for (int u : adj[midx][v]) {
    if (not used[u / 12] && place(u, nr, nc, midx)) return true;
  }
  placed[r][c] = -1; 
  used[v / 12] = false;
  return false;
...
int main() {
  ...
  vector<string> in = input([&](auto&) {});
  map<int, vector<string>> ps;
  for (int i = 0; i < SZ(in); i += 11) {
    int n = stoi(in[i]);
  ...
  for (auto& [n, vs] : ps) {
    F0R (k, 3) {
      if (k == 1) reverse(ALL(vs));
      if (k == 2) for (auto& s : vs) reverse(ALL(s));
      F0R (_, 4) {
        ts.pb(vs);
        num.pb(n);
        rotate(vs);
  ...
  adj.assign(4, vector<set<int>>(N));
  used.assign(N / 12, false);
  while (L * L * 12 < N) ++L;
  placed.assign(L, vi(L, -1));
  F0R (i, N) {
    F0R (j, N) {
      if (i / 12 == j / 12) continue;
      bool ok = true;
      F0R (k, 10) {
        if (ts[i][k][9] != ts[j][k][0]) ok = false;
      }
      if (ok) adj[0][i].insert(j);
      if (ok) adj[2][j].insert(i);
      ok = true;
      F0R (k, 10) {
        if (ts[i][9][k] != ts[j][0][k]) ok = false;
      }
      if (ok) adj[1][i].insert(j);
      if (ok) adj[3][j].insert(i);
    }
  }
  ...
  cout << (ll)num[placed[0][0]] * num[placed[0][L - 1]] * num[placed[L - 1][0]] * num[placed[L - 1][L - 1]] << endl;

  vector<string> img(L * 8, string(L * 8, '?'));
  F0R (R, L) {
    F0R (C, L) {
      int idx = placed[R][C];
      F0R (r, 8) {
        F0R (c, 8) {
          img[R * 8 + r][C * 8 + c] = ts[idx][r + 1][c + 1];
        }
      }
    }
  }
  ...
  vector<vector<bool>> marked(L * 8, vector<bool>(L * 8));
  F0R (k, 3) {
    if (k == 1) reverse(ALL(img));
    if (k == 2) for (auto& s : img) reverse(ALL(s));
    F0R (_, 4) {
      // cout << pp(img, "\n", "") << endl << endl;
      F0R (R, SZ(img) - SZ(p)) {
        F0R (C, SZ(img) - SZ(p[0])) {
          F0R (r, SZ(p)) {
            F0R (c, SZ(p[0])) {
              if (p[r][c] == '#' && img[R + r][C + c] != p[r][c]) goto fail;
            }
          }
          F0R (r, SZ(p)) {
            F0R (c, SZ(p[0])) {
              if (p[r][c] == '#') marked[R + r][C + c] = true;
            }
          }
        fail:;
        }
  ...
  int cnt = 0;
  for (auto& v : img) cnt += count(ALL(v), '#');
  for (auto& v : marked) cnt -= count(ALL(v), true);
  cout << cnt << endl;
