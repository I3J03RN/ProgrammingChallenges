...
void solve1(const vector<string>& ss) {
  vector<vector<string>> rr(13, vector<string>(20, string(20, '.')));
  F0R (r, SZ(ss)) {
    F0R (c, SZ(ss[r])) {
      rr[6][r + 6][c + 6] = ss[r][c];
    }
          ...
          FOR (xx, x - 1, x + 2) {
            FOR (yy, y - 1, y + 2) {
              FOR (zz, z - 1, z + 2) {
                ...
                if (min({yy, xx, zz}) < 0 || xx >= 13 || max(yy, zz) >= 20) continue;
                cnt += rr[xx][yy][zz] == '#';
              }
            }
          }
          if (rr[x][y][z] == '#')  {
            if (cnt < 2 || cnt > 3) nn[x][y][z] = '.';
          } else {
            if (cnt == 3) nn[x][y][z] = '#';
          }
        }
  ...
  int sum = 0;
  for (auto& v1 : rr) for (auto& v2 : v1) sum += count(ALL(v2), '#');
  cout << sum << endl;
}

void solve2(const vector<string>& ss) {
  vector<vector<vector<string>>> rr(13, vector<vector<string>>(13, vector<string>(20, string(20, '.'))));
  F0R (r, SZ(ss)) {
    F0R (c, SZ(ss[r])) {
      rr[6][6][r + 6][c + 6] = ss[r][c];
    }
  }
  F0R (_, 6) {
    auto nn = rr;
          ...
          F0R (z, 20) {
            ...
            FOR (xx, x - 1, x + 2) {
              FOR (ww, w - 1, w + 2) {
                FOR (yy, y - 1, y + 2) {
                  FOR (zz, z - 1, z + 2) {
                    if (xx == x && ww == w && yy == y && zz == z) continue;
                    if (min({yy, ww, xx, zz}) < 0 || max(xx, ww) >= 13 || max(yy, zz) >= 20) continue;
                    cnt += rr[xx][ww][yy][zz] == '#';
                  }
                }
              }
  ...
  int sum = 0;
  for (auto& v1 : rr) for (auto& v2 : v1) for(auto& v3 : v2) sum += count(ALL(v3), '#');
  cout << sum << endl;
}
