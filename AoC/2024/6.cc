...
int main() {
  ...
  auto mv = [&]() -> bool {
    if (see.count({pos, d})) return true;
    seen += pos;
    see += {pos, d};
    if (auto p = pos + dir[d]; ok(p) and gt(p) == '#') {
      d = (d + 1) % 4;
    } else {
      pos += dir[d];
    }
  ...
  auto test = [&](bool pr = false) -> bool {
    while (ok(pos)) {
      if (mv()) return true;
      if (pr) cout << dvarb(pos, d) << endl;
    }
    return false;
  };
