...
int main() {
  ...
  auto calc = [](ll ax, ll ay, ll bx, ll by, ll x, ll y) -> ll {
    // x += 10000000000000ll; y += 10000000000000ll;
    ll u = y * ax - ay * x, l = by * ax - ay * bx;
    assert(l != 0);
    if (u % l) return 0;
    ll b = u / l;
    if ((y - by * b) % ay) return 0;
    ll a = (y - by * b) / ay;
    assert(ax * a + bx * b == x and ay * a + by * b == y);
    return 3 * a + b;
  };
  ...
  input([&](auto & ss) {
    if (i == 0) {
      ss >> s >> s >> c >> c >> ax >> c >> c >> c >> ay;
    } else if (i == 1) {
      ss >> s >> s >> c >> c >> bx >> c >> c >> c >> by;
    } else if (i == 2) {
      ss >> s >> c >> c >> x >> c >> c >> c >> y;
      sc += calc(ax, ay, bx, by, x, y);
      scc += calc(ax, ay, bx, by, 10000000000000ll + x, 10000000000000ll + y);
    }
    i = (i + 1) % 4;
  });
