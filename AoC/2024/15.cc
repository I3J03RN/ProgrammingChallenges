...
map<char, ii> dir{{'<', {0, -1}}, {'^', {-1, 0}}, {'>', {0, 1}}, {'v', {1, 0}}};
...
int main() {
  ...
  auto ok = [&](ii a, char d) -> bool {
    ii v = dir[d];
    F0R (i, 2) {
      if (hs.count(a + v + i * mp(0, 1))) return false;
    }
    return true;
  };

  auto fb = [&](ii p, char d, int ign = -1) -> int {
    F0R (i, SZ(bs)) {
      if (i == ign) continue;
      F0R (j, 2) {
        if (p + dir[d] == bs[i] + j * mp(0, 1)) return i;
      }
    }
    return -1;
  };

  auto mvt = [&](ii p, char d) -> ii {
    ii v = dir[d];
    if (hs.count(p + v)) return p;
    if (int b = fb(p, d); b != -1) {
      vi bb; bb.pb(b); set<int> seen; seen += b; seen += -1;
      F0R (i, SZ(bb)) {
        F0R (j, 2) {
          if (int k = fb(bs[bb[i]] + j * mp(0, 1), d, bb[i]); not seen.count(k)) {
            bb.pb(k);
            seen += k;
          }
        }
  ...
  auto mv = [&](ii p, char d) -> ii {
    ii v = dir[d];
    for (int i = 1; gt(p + i * v) != '#'; ++i) {
      if (gt(p + i * v) == '.') {
        gt(p) = '.';
        gt(p + v) = '@';
        for (int j = 2; j <= i; ++j) gt(p + i * v) = 'O';
        return p + v;
      }
    }
    return p;
  ...
  F0R (r, R) {
    F0R (c, C) {
      if (f[r][c] == 'O') sc += 100 * r + c;
    }
  }

  cout << sc << endl << scc << endl;
