...
int main() {
  ...
  for (int i = SZ(bs) - 1; i >= 0; --i) {
    int mvto = -1;
    for (int j = bs[i].se; j <= 9; ++j) {
      if (SZ(fr[j]) and ckmin(bs[i].fi, *fr[j].begin())) mvto = j;
    }
    if (mvto != -1) {
      fr[mvto - bs[i].se] += *fr[mvto].begin() + bs[i].se;
      fr[mvto].erase(fr[mvto].begin());
    }
  }
  res = 0;
  F0R (i, SZ(bs)) {
