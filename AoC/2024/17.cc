...
int main() {
  ...
  auto calc = [&](ll a, ll b, ll c) -> vi {
    rs[0] = a;
    rs[1] = b;
    rs[2] = c;
    vi out;

    for (int i = 0; i < SZ(ins); i += 2) {
      switch (ins[i]) {
      case 0:
        if (ins[i + 1] < 4) {
          rs[0] = rs[0] >> ins[i + 1];
        } else {
          rs[0] = rs[0] >> rs[ins[i + 1] - 4];
        }
        break;
      case 1:
        rs[1] ^= ins[i + 1];
  ...
  cout << pp(calc(rs[0], rs[1], rs[2]), ",") << endl;

  auto fi = y_combinator([&](auto self, int i, ll a, int s = 0) -> ll {
    if (i < 0) return a;
    FOR (k, s, 8) {
      ll aa = (a << 3) | k;
      if (((aa >> (k ^ 3)) ^ 6 ^ k) % 8 == ins[i]) {
        if (ll res = self(i - 1, aa); res != -1) return res;
      }
    }
    return -1;
  });

  cout << fi(SZ(ins) - 1, 0, 1) << endl;
