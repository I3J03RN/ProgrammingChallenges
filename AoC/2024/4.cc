...
int main() {
  ...
  auto calc = [](vector<string> ss, string ne) -> int {
    int cnt = 0;
    for (auto& s : ss) {
      size_t p = 0;
      while (true) {
        auto pos = s.find(ne, p);
        if (pos != s.npos) {
          ++cnt; p = pos + 1;
        } else {
          break;
  ...
  auto trans = [](vector<string> ss) -> vector<string> {
    vector<string> out(SZ(ss[0]), string(SZ(ss), '.'));
    F0R (i, SZ(out)) {
      F0R (j, SZ(out[i])) {
        out[i][j] = ss[j][i];
      }
  ...
  auto rot = [](vector<string> ss) -> vector<string> {
    vector<string> out(SZ(ss) + SZ(ss[0]), string(min(SZ(ss), SZ(ss[0])), '.'));
    vi idx(SZ(ss) + SZ(ss[0]));
    F0R (j, SZ(ss[0])) {
      F0R (i, SZ(ss)) {
        out[j + i][idx[j + i]++] = ss[i][j];
      }
    }
    return out;
  };
  ...
  auto r = rot(ss);
  int cnt = calc(ss, "XMAS") + calc(ss, "SAMX") + calc(r, "XMAS") + calc(r, "SAMX");
  r = ss;
  ss = trans(ss);
  for (auto& s : r) reverse(ALL(s));
  r = rot(r);
  cnt += calc(ss, "XMAS") + calc(ss, "SAMX") + calc(r, "XMAS") + calc(r, "SAMX");
  cout << cnt << endl << cntt << endl;
