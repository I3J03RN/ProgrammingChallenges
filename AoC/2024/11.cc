...
int main() {
  ...
  F0R (_, 25) {
    ...
    for (auto a : as) {
      if (a == 0) {
        bs.pb(1);
      } else if (auto s = to_string(a); SZ(s) % 2 == 0) {
        ll bt = pow(10, SZ(s) / 2);
        bs.pb(a / bt);
        bs.pb(a % bt);
      } else {
  ...
  auto calc = y_combinator([&](auto self, ll v, int st) -> ll {
    if (st == 0) return 1;
    if (dp.count({v, st})) return dp[{v, st}];
    if (v == 0) {
      return dp[{v, st}] = self(1, st - 1);
    } else if (auto s = to_string(v); SZ(s) % 2 == 0) {
      ll bt = pow(10, SZ(s) / 2);
      ll res = 0;
      if (dp.count({v / bt, st - 1})) res += dp[{v / bt, st - 1}];
      else res += self(v / bt, st - 1);
      if (dp.count({v % bt, st - 1})) res += dp[{v % bt, st - 1}];
      else res += self(v % bt, st - 1);
      return dp[{v, st}] = res;
    } else {
      return dp[{v, st}] = self(v * 2024, st - 1);
    }
  });

  ll cnt = 0;
  for (auto c : cs) cnt += calc(c, 75);
  cout << cnt << endl;
