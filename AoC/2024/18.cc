...
int main() {
  ...
  vii cs;
  input([&](auto& ss) {
    int x, y; char c; ss >> x >> c >> y;
    cs.eb(y, x);
  });

  int E = SZ(cs) < 100 ? 7 : 71;
  int c = SZ(cs) < 100 ? 12 : 1024;

  array<ii, 4> ds{{{1, 0}, {0, 1}, {-1, 0}, {0, -1}}};

  auto find = [&](ii s, ii e, int n) -> int {
    queue<ii> q; q.push(s);
    ...
    while (SZ(q)) {
      ...
      F0R (d, 4) {
        ii vv = v + ds[d];
        if (0 <= min(vv.fi, vv.se) and max(vv.fi, vv.se) < E and not dt.count(vv)) {
          q.push(vv);
          dt[vv] = dt[v] + 1;
        }
      }
  ...
  ii s{0, 0};
  ii e = SZ(cs) < 100 ? mp(6, 6) : mp(70, 70);

  cout << find(s, e, c) << endl;
