...
int main() {
  ...
  vector<string> ss = input([](auto&) {});

  auto num = [](string& n) -> bool {
    for (auto c : n) if (c < '0' or '9' < c) return false;
    return true;
  };
  ...
  auto calc = [&](string s, bool dodis = false) -> ll {
    ll sol = 0;
    F0R (i, SZ(s)) {
      if (s.substr(i).starts_with("don't()")) {
        if (dodis) disable = true;
      } else if (s.substr(i).starts_with("do()")) {
        disable = false;
      } else if (s.substr(i).starts_with("mul(")) {
        auto ap = s.find(",", i + 4);
        if (ap == s.npos) continue;
        string a(s.begin() + i + 4, s.begin() + ap);
        if (not num(a)) continue;
        auto bp = s.find(")", ap + 1);
        if (bp == s.npos) continue;
        string b(s.begin() + ap + 1, s.begin() + bp);
        if (not num(b)) continue;
        if (not disable) {
          sol += stoll(a) * stoll(b);
        }
      }
  ...
  cout << calc("xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))") << endl;
  cout << calc("xmul(2,4)&mul[3,7]!^don't()_mul(5,5)+mul(32,64](mul(11,8)undo()?mul(8,5))", true) << endl;

  disable = false;

  ll sol = 0, soll = 0;
  for (auto& s : ss) sol += calc(s);
  for (auto& s : ss) soll += calc(s, true);
  cout << sol << endl << soll << endl;
