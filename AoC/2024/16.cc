...
array<ii, 4> dirs{{{0, 1}, {-1, 0}, {0, -1}, {1, 0}}};
...
int main() {
  ...
  ii s = {SZ(f) - 2, 1}, e = {1, SZ(f[1]) - 2};

  auto gt = [&](ii p) -> char& {
    return f[p.fi][p.se];
  };

  map<pair<ii, int>, int> dist;
  set<tuple<int, ii, int>> q;
  dist[{s, 0}] = 0;
  q.emplace(0, s, 0);
  while (SZ(q)) {
    auto [d, p, dir] = *q.begin(); q.erase(q.begin());
    if (d > dist[{p, dir}]) continue;
    if (gt(p + dirs[dir]) != '#' and (not dist.count({p + dirs[dir], dir}) or d + 1 < dist[{p + dirs[dir], dir}])) {
      dist[{p + dirs[dir], dir}] = d + 1;
      q.emplace(d + 1, p + dirs[dir], dir);
    }
    for (int i : {-1, 1}) {
      int dd = (dir + i + 4) % 4;
      if (not dist.count({p, dd}) or d + 1000 < dist[{p, dd}]) {
        dist[{p, dd}] = d + 1000;
        q.emplace(d + 1000, p, dd);
      }
    }
  }

  int sc = min(dist[{e, 0}], dist[{e, 1}]);

  cout << sc << endl;

  set<pair<ii, int>> seen;
  vector<pair<ii, int>> besties;
  F0R (i, 4) {
    if (dist[{e, i}] == sc) {
      seen += {e, i};
      besties.eb(e, i);
  ...
  F0R (i, SZ(besties)) {
    auto [p, d] = besties[i];
    int dd = (d + 2) % 4;
    if (gt(p + dirs[dd]) != '#' and dist[{p + dirs[dd], d}] + 1 == dist[{p, d}] and not seen.count({p + dirs[dd], d})) {
      seen.emplace(p + dirs[dd], d);
      besties.eb(p + dirs[dd], d);
    }
    for (int j : {-1, 1}) {
      dd = (d + 4 + j) % 4;
      if (dist[{p, dd}] + 1000 == dist[{p, d}] and not seen.count({p, dd})) {
        seen.emplace(p, dd);
        besties.eb(p, dd);
      }
    }
