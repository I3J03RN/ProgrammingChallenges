...
int main() {
  ...
  auto dist = [&](ii s) -> map<ii, int> {
    queue<ii> q; q.push(s);
    map<ii, int> dt; dt[s] = 0;
    while (SZ(q)) {
      ii v = q.front(); q.pop();
      F0R (i, 4) {
        ii vv = v + ds[i];
        if (ok(vv) and gt(vv) != '#' and not dt.count(vv)) {
          q.push(vv);
          dt[vv] = dt[v] + 1;
        }
      }
  ...
  auto cnt = [&](int ct, int imp) -> int {
    int sc = 0;
    map<ii, int> ds = dist(s), de = dist(e);
    int b = ds[e];
    for (auto [p, d] : ds) {
      if (d <= b - imp) {
        for (int cr = -ct; cr <= ct; ++cr) {
          for (int cc = -ct + abs(cr); cc <= ct - abs(cr); ++cc) {
            ii pp = p + mp(cr, cc);
            sc += ok(pp) and gt(pp) != '#' and d + de[pp] + abs(cr) + abs(cc) <= b - imp;
          }
        }
      }
    }
