...
int main() {
  ...
  auto north = [](vector<string> in) -> vector<string> {
    FOR (i, 1, SZ(in)) {
      for (int j = i; j > 0; --j) {
        F0R (k, SZ(in[j])) {
          if (in[j][k] == 'O' and in[j - 1][k] == '.') swap(in[j][k], in[j - 1][k]);
        }
      }
    }
    return in;
  ...
  auto cycle = [&north](vector<string> in) {
    F0R (_, 4) {
      in = north(in);
      vector<string> nx(SZ(in[0]), string(SZ(in), '?'));
      F0R (r, SZ(in)) {
        F0R (c, SZ(in[r])) {
          nx[c][SZ(in) - 1 - r] = in[r][c];
        }
      }
      in.swap(nx);
    }
  ...
  auto calc = [](vector<string> in) {
    ll sm = 0;
    F0R (r, SZ(in)) {
      F0R (c, SZ(in[r])) {
        if (in[r][c] == 'O') sm += SZ(in) - r;
      }
    }
    return sm;
  };

  cout << calc(north(in)) << endl;
  ...
  for (int i = 1;; ++i) {
    ...
    if (seen.count(in)) {
      int cyc = i - seen[in];
      int f = (1000000000 - seen[in]) % cyc + seen[in];
      for (auto [m, v] : seen) {
        if (v == f) {
          cout << calc(m) << endl;
        }
