...
int main() {
  ...
  vector<vector<ll>> vvs;
  input([&](auto& ss) {
    vvs.eb();
    ll v;
    while (ss >> v) vvs.back().pb(v);
  });

  auto calc = y_combinator([](auto self, vector<ll> vs) -> pair<ll, ll> {
    if (count(ALL(vs), 0) == SZ(vs)) return {0, 0};
    vector<ll> nx;
    F0R (i, SZ(vs) - 1) {
      nx.pb(vs[i + 1] - vs[i]);
    }
