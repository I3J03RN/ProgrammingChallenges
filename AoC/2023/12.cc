...
int main() {
  ...
  auto calc = [](string s, vi vs, int mult = 1) -> ll {
    string sp = s;
    vi vsp = vs;
    F0R (_, mult - 1) {
      s += '?' + sp;
      vs.insert(vs.end(), ALL(vsp));
    }
    s += string(1 + *max_element(ALL(vs)), '.');
    vector<vector<ll>> dp(SZ(s) + 1, vector<ll>(SZ(vs) + 1, 0));
    dp[SZ(s)][SZ(vs)] = 1;
  ...
  ll sm = 0;
  F0R (i, SZ(rs)) sm += calc(rs[i], vs[i]);
  cout << sm << endl;

  sm = 0;
  F0R (i, SZ(rs)) sm += calc(rs[i], vs[i], 5);
  cout << sm << endl;
