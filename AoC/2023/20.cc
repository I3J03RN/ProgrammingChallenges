...
int main() {
  ...
  auto go = [&](bool& found) -> pair<ll, ll> {
    vector<ll> ps(2, 0);

    // to, value, from, fvalue
    queue<tuple<string, bool, string, bool>> q;
    q.emplace("broadcaster", 0, "", 0);

    while (not q.empty()) {
      auto [n, v, fn, fv] = q.front(); q.pop();
      ++ps[v];
      if (n == "rx" and not v) found = true;
      // cout << dvar(n, v, fn, fv) << endl;
      if (not type.count(n)) continue;
      if (type[n] == '%') {
        if (v) continue;
        mem[n][n] = not mem[n][n];
        for (auto t : targets[n]) {
          q.emplace(t, mem[n][n], n, v);
        }
      } else if (type[n] == '&') {
        mem[n][fn] = v;
        bool ok = true;
  ...
  auto gofind = [&]() -> ll {
    ...
    bool _ = false;
    for (ll i = 1;; ++i) {
      if (i % 1000000 == 0) cout << dvar(i) << endl;
      go(_);
      if (_) return i;
    }
  };
