...
int main() {
  ...
  for (int i = 0, f = 1; i < SZ(in); ++i) {
    if (in[i].empty()) {
      --f;
    } else if (f) {
      string n = in[i].substr(0, in[i].find("{"));
      ins[n] = split(string(SZ(n) + 1 + ALL(in[i]) - 1));
    } else {
      string s(1 + ALL(in[i]) - 1);
      ps.eb();
      for (auto ss : split(string(1 + ALL(in[i]) - 1))) {
        ps.back()[ss[0]] = stoll(ss.substr(2));
      }
    }
  }

  auto go = y_combinator([&ins](auto self, map<char, ll> p, string i) -> ll {
    if (i == "R") return 0;
    if (i == "A") {
      ll sm = 0;
      for (auto [_, v] : p) sm += v;
    ...
    for (auto s : ins[i]) {
      if (auto c = s.find(":"); c != string::npos) {
        char cmp = s[1];
        ll v = stoll(s.substr(2, c - 2));
        if (cmp == '<') {
          if (p[s[0]] < v) return self(p, s.substr(c + 1));
        } else if (cmp == '>') {
          if (p[s[0]] > v) return self(p, s.substr(c + 1));
        } else {
          cout << "huh?" << dvar(cmp) << endl;
          assert(false);
        }
      } else {
        return self(p, s);
    ...
    cout << "nothing for " << dvar(p, i, ins[i]) << endl;
    return 0;
  });
  ...
  auto go2 = y_combinator([&ins](auto self, map<char, ii> rs, string i) -> ll {
    if (i == "R") return 0;
    if (i == "A") {
      ll sm = 1;
      for (auto [_, v] : rs) sm *= max(v.se - v.fi + 1, 0);
      return sm;
    }
    ll res = 0;
    for (auto s : ins[i]) {
      if (auto c = s.find(":"); c != string::npos) {
        char cmp = s[1];
        int v = stoll(s.substr(2, c - 2));
        if (cmp == '<') {
          auto rsp = rs;
          auto [l, h] = rs[s[0]];
          rsp[s[0]] = {l, min(v - 1, h)};
          res += self(rsp, s.substr(c + 1));
          rs[s[0]] = {max(v, l), h};
        } else if (cmp == '>') {
          auto rsp = rs;
          auto [l, h] = rs[s[0]];
  ...
  map<char, ii> rs;
  for (char c : string("asmx")) rs[c] = {1, 4000};
  cout << go2(rs, "in") << endl;
