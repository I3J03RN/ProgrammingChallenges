...
int main() {
  ...
  ll sm = 0;
  ll sm2 = 0;
  auto calc = [](const vector<string>& pat, int c = 0) -> int {
    F0R (j, SZ(pat) - 1) {
      int cnt = 0;
      for (int b = j, a = j + 1; b >= 0 and a < SZ(pat); --b, ++a) {
        F0R (i, SZ(pat[b])) cnt += pat[b][i] != pat[a][i];
      }
      if (cnt == c) {
        return j + 1;
      }
  ...
  F0R (i, SZ(in) + 1) {
    if (i == SZ(in) or in[i] == "") {
      sm += calc(pat) * 100;
      sm2 += calc(pat, 1) * 100;
      vector<string> nx(SZ(pat[0]), string(SZ(pat), '?'));
      F0R (r, SZ(pat)) F0R (c, SZ(pat[0])) nx[c][r] = pat[r][c];
      pat.swap(nx);
      sm += calc(pat);
      sm2 += calc(pat, 1);
      pat.clear();
