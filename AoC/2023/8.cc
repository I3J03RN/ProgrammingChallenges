...
int main() {
  ...
  vector<string> curs;
  FOR (i, 2, SZ(in)) {
    if (in[i][2] == 'A') curs.pb(in[i].substr(0, 3));
    l[in[i].substr(0, 3)] = in[i].substr(7, 3);
    r[in[i].substr(0, 3)] = in[i].substr(12, 3);
  }
  string cur = "AAA";
  for (int i = 0;; ++i) {
    cur = (ins[i % SZ(ins)] == 'L' ? l : r)[cur];
    if (cur == "ZZZ") {
      cout << i + 1 << endl;
      break;
    }
  ...
  // offset, length, zpos
  auto re = [&](string s) -> tuple<int, int, vi> {
    map<pair<string, int>, int> seen; seen[{s, 0}] = 0;
    vi zs;
    string c = s;
    for (int i = 0;; ++i) {
      c = (ins[i % SZ(ins)] == 'L' ? l : r)[c];
      if (auto it = seen.find({c, (i + 1) % SZ(ins)}); it != seen.end()) {
        assert(SZ(zs) == 1);
        return {it->se, i + 1, zs};
      }
      if (c[2] == 'Z') zs.pb(i + 1);
      seen[{c, (i + 1) % SZ(ins)}] = i + 1;
    }
  };
