...
int main() {
  ...
  auto calc = [&](int mn, int mx) -> int {
    ...
    while (not q.empty()) {
      auto [d, x, y, ld] = *q.begin(); q.erase(q.begin());
      if (dist[{x, y, ld}] < d) continue;
      if (x == SZ(in[0]) - 1 and y == SZ(in) - 1) return d;
      F0R (i, 4) {
        if (i % 2 == ld % 2) continue;
        int dd = d;
        FOR (j, 1, mx + 1) {
          int xx = x + j * xd[i], yy = y + j * yd[i];
          if (min(xx, yy) < 0 or xx >= SZ(in[0]) or yy >= SZ(in)) continue;
          dd += in[yy][xx] - '0';
          if (j < mn) continue;
          if (not dist.count({xx, yy, i}) or dist[{xx, yy, i}] > dd) {
            dist[{xx, yy, i}] = dd;
            q.emplace(dd, xx, yy, i);
          }
        }
      }
    }
