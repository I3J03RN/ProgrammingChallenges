...
int main() {
  ...
  in.insert(in.begin(), string(SZ(in[0]), '.'));
  in.pb(string(SZ(in[0]), '.'));
  ...
  ii up{0, -1}, down{0, 1}, left{-1, 0}, right{1, 0};
  vii a{up, down, left, right};

  map<char, vector<ii>> dir{
    {'-', {left, right}},
    {'|', {up, down}},
    {'F', {right, down}},
    {'L', {right, up}},
  ...
  auto ne = [&](ii xy) -> vii {
    vii out;
    if (auto it = dir.find(in[xy.se][xy.fi]); it != dir.end()) {
      for (auto d : it->se) out.pb(xy + d);
    }
    return out;
  };
