...
int main() {
  ...
  vector<ll> seeds;
  vector<vector<tuple<ll, ll, ll>>> trans;
  auto in = input([&](auto&) {});
  F0R (i, SZ(in)) {
    ...
    if (i == 0) {
      string s; ss >> s;
      ll v;
      while (ss >> v) seeds.pb(v);
    } else {
      if (in[i] == "") {
        trans.pb({}); ++i;
      } else {
        tuple<ll, ll, ll> t; ss >> t; trans.back().pb(t);
      }
    }
  }

  auto calc = [&](ll v) -> ll {
    for (const auto& vt : trans) {
      for (const auto& [b, a, c] : vt) {
        if (a <= v and v < a + c) {
          v += b - a;
          break;
        }
  ...
  auto calcr = [&](vector<pair<ll, ll>> rs) -> vector<pair<ll, ll>> {
    for (const auto& vt : trans) {
      ...
      F0R (i, SZ(rs)) {
        ...
        bool found = false;
        for (const auto& [b, a, c] : vt) {
          ll mxl = max(a, s), mnr = min(a + c, s + r);
          if (mxl < mnr) {
            nx.eb(mxl + b - a, mnr - mxl);
            if (mxl > s) {
              rs.eb(s, mxl - s);
            }
            if (mnr < a + c) {
  ...
  for (int i = 0; i < SZ(seeds); i += 2) {
    rs.eb(seeds[i], seeds[i + 1]);
  }
