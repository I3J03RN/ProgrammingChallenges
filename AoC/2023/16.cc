...
int main() {
  ...
  auto in = input([&](auto&) {});
  ii left = {-1, 0}, right = {1, 0}, up = {0, -1}, down = {0, 1};
  vii dirs{left, right, up, down};
  map<ii, set<ii>> marked;

  auto dfs = y_combinator([&](auto self, ii p, ii d) -> void {
    p += d;
    auto [x, y] = p;
    if (min(x, y) < 0 or y >= SZ(in) or x >= SZ(in[0])) return;
    if (marked[p] < d) return;
    marked[p] += d;
    switch(in[y][x]) {
    case '.':
      self(p, d);
    ...
    case '-':
      if (d == left or d == right) {
        ...
        for (auto dd : {left, right}) self(p, dd);
      }
      break;
    case '|':
      if (d == down or d == up) {
        ...
        for (auto dd : {down, up}) self(p, dd);
      }
      break;
    case '\\':
      {
        array<ii, 4> a{left, down, up, right};
        F0R (i, 4) {
          if (d == a[i]) {
            self(p, a[(i + 2) % 4]);
            break;
    ...
    case '/':
      {
        array<ii, 4> a{right, down, up, left};
        F0R (i, 4) {
          if (d == a[i]) {
            self(p, a[(i + 2) % 4]);
            break;
  ...
  auto cnt = [&](ii p, ii d) -> int {
    marked.clear();
    dfs(p - d, d);
    int sm = 0;
    F0R (r, SZ(in)) {
      F0R (c, SZ(in[r])) {
        sm += not not SZ(marked[mp(c, r)]);
      }
    }
    return sm;
  };

  cout << cnt(mp(0, 0), right) << endl;

  int mx = 0;
  F0R (r, SZ(in)) {
    ckmax(mx, cnt(mp(0, r), right));
    ckmax(mx, cnt(mp(SZ(in[r]) - 1, r), left));
  }
  F0R (c, SZ(in[0])) {
    ckmax(mx, cnt(mp(c, 0), down));
    ckmax(mx, cnt(mp(c, SZ(in) - 1), up));
  }
  cout << mx << endl;
