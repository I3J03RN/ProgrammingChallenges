...
int main() {
  ...
  auto sym = [&](int r, int c) -> vii {
    vii out;
    for (int xr : {-1, 0, 1}) {
      for (int xc : {-1, 0, 1}) {
        if (xr == 0 and xc == 0) continue;
        int rr = r + xr, cc = c + xc;
        if (0 <= rr and rr < SZ(in) and 0 <= cc and cc < SZ(in[rr]) and in[rr][cc] != '.' and not isdigit(in[rr][cc])) {
          out.eb(rr, cc);
        }
      }
    }
  ...
  F0R (r, SZ(in)) {
    ...
    set<ii> syms;
    F0R (c, SZ(in[r]) + 1) {
      if (c < SZ(in[r]) and isdigit(in[r][c])) {
        v = v * 10 + (in[r][c] - '0');
        auto nsyms = sym(r, c);
        syms.insert(ALL(nsyms));
      } else {
        if (not syms.empty()) sm += v;
        for (auto s : syms) ns[s].pb(v);
        v = 0;
        syms.clear();
      }
    }
