...
int main() {
  ...
  auto in = input([&](auto&) {})[0] + ",";
  string s;
  int sm = 0;
  vector<vector<pair<string, int>>> ls(256);
  for (char c : in) {
    if (c == ',') {
      sm += hash(s);

      if (s.back() == '-') {
        string la = s.substr(0, SZ(s) - 1);
        auto& vs = ls[hash(la)];
        F0R (i, SZ(vs)) {
          if (vs[i].fi == la) {
            vs.erase(vs.begin() + i);
