...
int main() {
  ...
  map<char, ii> dir{{'R', {1, 0}}, {'L', {-1, 0}}, {'U', {0, -1}}, {'D', {0, 1}}};

  auto calc = [&](string ds, vi ns) -> ll {
    ll res = 0;
    vector<pair<ii, ii>> es;
    ll mny = 0, mxy = 0;
    ii c{0, 0};
    F0R (i, SZ(ds)) {
      ii cc = c + ns[i] * dir[ds[i]];
      ckmin(mny, (ll)cc.se); ckmax(mxy, (ll)cc.se);
      es.eb(c, cc);
      c = cc;
    }
    FOR (y, mny, mxy + 1) {
      ...
      for (auto [a, b] : es) {
        ...
        auto [xb, yb] = b;
        if (ya != yb) { // |
          if (min(ya, yb) <= y and y <= max(ya, yb)) {
            ps.eb(xa, ya < yb ? 1 : -1);
          }
        } else { // -
          if (ya == y) ps.eb(min(xa, xb) + 1, max(xa, xb) - 1);
        }
      }
      sort(ALL(ps));
      F0R (i, SZ(ps) - 1) {
        if (ps[i].se == ps[i + 1].se) {
          ps.erase(ps.begin() + i + (1 - i % 2));
          --i;
        } else if (ps[i].fi + 1 == ps[i + 1].fi) {
          if (i % 2 and ps[i].se != ps[i + 2].se) {
            sm += ps[i + 1].se - ps[i + 1].fi + 1;
          }
  ...
  input([&](auto& ss) {
    char c; int n; string s;
    ss >> c >> n;
    ds += c;
    ns.pb(n);
    ss >> c >> c >> s;
    dss += "RDLU"[s[5] - '0'];
    stringstream strs(s.substr(0, 5));
    strs >> hex >> n;
    nss.pb(n);
  });

  cout << calc(ds, ns) << endl;
  cout << calc(dss, nss) << endl;
