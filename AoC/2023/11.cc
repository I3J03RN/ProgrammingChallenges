...
int main() {
  ...
  vector<bool> mr(SZ(in), false), mc(SZ(in[0]), false);
  F0R (i, SZ(in)) {
    if (count(ALL(in[i]), '.') == SZ(in[i])) {
      mr[i] = true;
    }
  }
  ...
  F0R (i, SZ(in[0])) {
    int cnt = 0;
    F0R (j, SZ(in)) cnt += in[j][i] == '.';
    if (cnt == SZ(in)) {
      mc[i] = true;
    }
  }

  array<int, 4> xd{1, 0, -1, 0}, yd{0, 1, 0, -1};

  auto bfs = [&](int sx, int sy, ll exd = 2) -> ll {
    ll sm = 0;
    map<ii, ll> d;
    queue<ii> q;
    q.emplace(sx, sy);
    ...
    while (not q.empty()) {
      ...
      F0R (i, 4) {
        ...
        if (0 <= min(xx, yy) and yy < SZ(in) and xx < SZ(in[yy]) and not d.count({xx, yy})) {
          q.emplace(xx, yy);
          d[{xx, yy}] = d[{x, y}] + ((i % 2 == 0 and mc[x]) or (i % 2 == 1 and mr[y]) ? exd : 1);
        }
      }
    }
    return sm;
