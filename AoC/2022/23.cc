...
int main() {
  ...
  input([&](auto& in) {
    string s; in >> s;
    F0R (j, SZ(s)) if (s[j] == '#') ps += {j, i};
    ++i;
  });

  auto cnt = [&](int x, int y, int xd, int yd) -> int {
    int k = 0;
    for (int d : {-1, 0, 1}) k += ps.count({x + (xd == 0 ? d : xd), y + (yd == 0 ? d : yd)});
    return k;
  };
  ...
  for (int _ = 0;; ++_) {
    ...
    for (auto [x, y] : ps) {
      ...
      for (int xd : {-1, 0, 1}) {
        for (int yd : {-1, 0, 1}) {
          k += ps.count({x + xd, y + yd});
        }
      }
      if (k > 1) {
        F0R (t, 4) {
          int v = (t + _) % 4;
          if (v == 0 and not cnt(x, y, 0, -1)) {
            prop[{x, y - 1}].eb(x, y); break;
          } else if (v == 1 and not cnt(x, y, 0, 1)) {
            prop[{x, y + 1}].eb(x, y); break;
          } else if (v == 2 and not cnt(x, y, -1, 0)) {
            prop[{x - 1, y}].eb(x, y); break;
          } else if (v == 3 and not cnt(x, y, 1, 0)) {
            prop[{x + 1, y}].eb(x, y); break;
          }
        }
      }
    ...
    if (_ == 9) {
      int xmn, xmx, ymn, ymx;
      tie(xmn, ymn) = *ps.begin();
      xmx = xmn; ymx = ymn;
      for (auto [x, y] : ps) {
        ckmax(xmx, x); ckmin(xmn, x);
        ckmax(ymx, y); ckmin(ymn, y);
      }
      cout << (xmx - xmn + 1) * (ymx - ymn + 1) - SZ(ps) << endl;
    }
    if (not change and _ >= 9) {
      cout << _ + 1 << endl;
      break;
    };
  }
