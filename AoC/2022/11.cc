...
int main() {
  ...
  vector<vector<ll>> is;
  vector<function<ll(ll)>> op;
  vector<function<bool(ll)>> test;
  vi t, f;
  ...
  input([&](auto& in) {
    ...
    if (idx == 2) {
      ...
      op.eb([=](ll old) -> ll {
        if (o == "+") {
          return (a == "old" ? old : stoll(a)) + (b == "old" ? old : stoll(b));
        } else {
          return (a == "old" ? old : stoll(a)) * (b == "old" ? old : stoll(b));
        }
      });
    ...
    if (idx == 3) {
      ll val; in >> s >> s >> s >> val; mod *= val;
      test.eb([=](ll v) -> bool { return v % val == 0; });
    }
    if (idx == 4) {
      int x; in >> s >> s >> s >> s >> s >> x;
      t.pb(x);
    }
    if (idx == 5) {
      int x; in >> s >> s >> s >> s >> s >> x;
  ...
  F0R (_, 20) {
    F0R (i, N) {
      for (ll v : is[i]) {
        ++seen[i];
        v = op[i](v) / 3;
        is[(test[i](v) ? t : f)[i]].pb(v);
      }
      is[i].clear();
    }
  }
  sort(RALL(seen));
  cout << seen[0] * seen[1] << endl;
