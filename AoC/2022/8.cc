...
int main() {
  ...
  auto in = input([&](auto&) {});
  const int R = SZ(in), C = SZ(in[0]);
  vvi vis(R, vi(C, 4));
  F0R (r, R) {
    char mxl = '0' - 1, mxr = '0' - 1;
    F0R (c, C) {
      int cc = C - c - 1;
      if (mxl >= in[r][c]) --vis[r][c];
      ckmax(mxl, in[r][c]);
      if (mxr >= in[r][cc]) --vis[r][cc];
      ckmax(mxr, in[r][cc]);
    }
  }
  F0R (c, C) {
    ...
    F0R (r, R) {
      ...
      if (mxl >= in[r][c]) --vis[r][c];
      ckmax(mxl, in[r][c]);
      if (mxr >= in[rr][c]) --vis[rr][c];
      ckmax(mxr, in[rr][c]);
    }
  }
  int sm = 0;
  F0R (r, R) F0R (c, C) sm += !!vis[r][c];
  cout << sm << endl;
  ll best = 0;
  auto ok = [&](int r, int c) -> bool { return 0 <= min(r, c) and r < R and c < C; };
  auto calc = [&](int r, int c) -> ll {
    ll res = 1;
    for (auto [xr, xc] : vii{{1, 0}, {-1, 0}, {0, 1}, {0, -1}}) {
      ll row = 0;
      for (int rr = r + xr, cc = c + xc; ok(rr, cc); rr += xr, cc += xc) {
        ++row;
        if (in[rr][cc] >= in[r][c]) break;
      }
      res *= row;
    }
    return res;
  };
  F0R (r, R) F0R (c, C) ckmax(best, calc(r, c));
  cout << best << endl;
