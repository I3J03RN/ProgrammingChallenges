...
int main() {
  ...
  const int C = SZ(gd[0]), R = SZ(gd);
  const int M = lcm(C - 2, R - 2);

  ii s{1, 0}, e{C - 2, R - 1};

  vector<vector<vector<bool>>> fr(M, vector<vector<bool>>(R - 2, vector<bool>(C - 2, true)));
  F0R (r, R - 2) {
    F0R (c, C - 2) {
      F0R (m, M) {
        if (gd[r + 1][c + 1] == '>') fr[m][r][(c + m) % (C - 2)] = false;
        if (gd[r + 1][c + 1] == '<') fr[m][r][((c - m) % (C - 2) + C - 2) % (C - 2)] = false;
        if (gd[r + 1][c + 1] == 'v') fr[m][(r + m) % (R - 2)][c] = false;
        if (gd[r + 1][c + 1] == '^') fr[m][((r - m) % (R - 2) + R - 2) % (R - 2)][c] = false;
      }
    }
  }
  ...
  auto ok = [&](int c, int r, int m) -> bool {
    return 0 <= min(c, r) and c < C and r < R and gd[r][c] != '#' and (r == 0 or r == R - 1 or fr[m % M][r - 1][c - 1]) and not (seen < mp(mp(c, r), m % M));
  };

  auto go = [&](ii start, ii end, int im) -> int {
    q = {}; seen.clear();
    q.emplace(start, im); seen += {start, im};
    for (;;) {
      auto [v, m] = q.front(); q.pop();
      if (v == end) {
        return m;
      }
      auto [c, r] = v;
      for (int dc : {-1, 0, 1}) {
        for (int dr : {-1, 0, 1}) {
          if (abs(dc) + abs(dr) <= 1 and ok(c + dc, r + dr, m + 1)) {
            q.emplace(mp(c + dc, r + dr), m + 1);
            seen += {{c + dc, r + dr}, (m + 1) % M};
          }
        }
      }
    }
