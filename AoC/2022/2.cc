...
int main() {
  ...
  map<char, int> vs{{'X', 1}, {'Y', 2}, {'Z', 3}};
  auto ps = [](char a, char x) -> int {
    int n = a - 'A', m = x - 'X';
    if (n == m) return 3;
    n = (n + (1 - m) + 3) % 3;
    return 6 * (n < 1);
  };
  auto ps2 = [](char a, char x) -> char {
    int n = a - 'A';
    if (x == 'Y') return 'X' + static_cast<char>(n);
    if (x == 'X') return 'X' + static_cast<char>((n + 2) % 3);
    else return 'X' + static_cast<char>((n + 1) % 3);
  };
