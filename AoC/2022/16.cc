...
int main() {
  ...
  int aa = -1;
  input([&](auto& in) {
    string s, n; char c; int f;
    in >> s >> n;
    int i = geti(n);
    if (n == "AA") aa = i;
    in >> s >> s >> c >> c >> c >> c >> c >> f >> s >> s >> s >> s >> s;
    fw[i] = f;
    if (f != 0) pos[i] = npos++;
    while (in >> n) {
      int o = geti(n.substr(0, 2));
      adj[i].pb(o);
    }
  });
  ...
  {
    vector<vvi> dp(31, vvi(N, vi(1 << npos, -1)));
    dp[0][aa][0] = 0;
    F0R (m, 30) {
      F0R (v, N) {
        F0R (o, 1 << npos) {
          if (dp[m][v][o] == -1) continue;
          if (~pos[v] and not ((o >> pos[v]) & 1)) {
            ckmax(dp[m + 1][v][o | (1 << pos[v])], dp[m][v][o] + (30 - m - 1) * fw[v]);
          }
          for (int u : adj[v]) {
            ckmax(dp[m + 1][u][o], dp[m][v][o]);
          }
        }
      }
    }
    int best = 0;
    F0R (v, N) F0R (o, 1 << npos) ckmax(best, dp[26][v][o]);
    cout << best << endl;
  }

  {
    vector<vector<vvi>> dp(27, vector<vvi>(N, vvi(N, vi(1 << npos, -1))));
    dp[0][aa][aa][0] = 0;
    F0R (m, 26) {
      F0R (v, N) {
        F0R (p, N) {
          F0R (o, 1 << npos) {
            if (dp[m][v][p][o] == -1) continue;
            vector<tuple<int, int, int>> nv, np;
            if (~pos[v] and not ((o >> pos[v]) & 1)) {
              nv.eb(v, o | (1 << pos[v]), (26 - m - 1) * fw[v]);
            }
            if (p != v and ~pos[p] and not ((o >> pos[p]) & 1)) {
              np.eb(p, o | (1 << pos[p]), (26 - m - 1) * fw[p]);
            }
            for (int u : adj[v]) nv.eb(u, o, 0);
            for (int u : adj[p]) np.eb(u, o, 0);

            for (auto [vv, ov, vval] : nv) {
              for (auto [pp, op, pval] : np) {
                ckmax(dp[m + 1][vv][pp][ov | op], dp[m][v][p][o] + vval + pval);
              }
            }
          }
        }
    ...
    int best = 0;
    F0R (v, N) F0R (p, N) F0R (o, 1 << npos) ckmax(best, dp[26][v][p][o]);
    cout << best << endl;
  }
