...
int main() {
  ...
  int idx = 0;
  input([&](auto& in) {
    string s; int id, oreore, core, obore, obclay, gore, gob;
    in >> s >> id >> s >> s >> s >> s >> s >> oreore >> s >> s >> s >> s >> s >> core >> s >> s >> s >> s >> s >> obore >> s >> s >> obclay >> s >> s >> s >> s >> s >> gore >> s >> s >> gob;
    // ore, #orerobots, clay, #claybots, ob, #obbots, geode, #geodebots
    using key = tuple<int, int, int, int, int, int, int, int>;
    set<key> a, b;
    a.emplace(0, 1, 0, 0, 0, 0, 0, 0);
    int best = 0;
    F0R (_, (idx >= 3 ? 24 : 32)) {
      cout << dvar(_) << endl;
      for (auto [o, obots, c, cbots, ob, obbots, g, gbots] : a) {
        ckmax(best, g + gbots);
        // tested these values against the sample values under the assumption
        // that not spending ore is not optimal because every recipe needs it
        // and not spending ob holds our objective function back.
        if (o + obots >= 15 or ob + obbots >= 30 or g + gbots + 15 < best) continue;
        b.emplace(o + obots, obots, c + cbots, cbots, ob + obbots, obbots, g + gbots, gbots);
        if (o >= oreore) b.emplace(o - oreore + obots, obots + 1, c + cbots, cbots, ob + obbots, obbots, gbots + g, gbots);
        if (o >= core) b.emplace(o - core + obots, obots, c + cbots, cbots + 1, ob + obbots, obbots, gbots + g, gbots);
        if (o >= obore and c >= obclay) b.emplace(o - obore + obots, obots, c - obclay + cbots, cbots, ob + obbots, obbots + 1, gbots + g, gbots);
        if (o >= gore and ob >= gob) b.emplace(o - gore + obots, obots, c + cbots, cbots, ob - gob + obbots, obbots, gbots + g, gbots + 1);
