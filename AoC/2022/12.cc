...
int main() {
  ...
  while (!q.empty()) {
    ...
    auto [x, y] = v;
    for (auto [xd, yd] : vii{{1, 0}, {-1, 0}, {0, 1}, {0, -1}}) {
      int xx = x + xd, yy = y + yd;
      dout << dvar(x, y, xx, yy) << endl;;
      if (0 <= min(xx, yy) and xx < R and yy < C and m[xx][yy] <= m[x][y] + 1 and not (seen < mp(xx, yy))) {
        q.emplace(mp(xx, yy), d + 1);
        seen += {xx, yy};
      }
    }
  }
  ...
  while (!q.empty()) {
    ...
    for (auto [xd, yd] : vii{{1, 0}, {-1, 0}, {0, 1}, {0, -1}}) {
      int xx = x + xd, yy = y + yd;
      dout << dvar(x, y, xx, yy) << endl;;
      if (0 <= min(xx, yy) and xx < R and yy < C and m[x][y] <= m[xx][yy] + 1 and not (seen < mp(xx, yy))) {
        q.emplace(mp(xx, yy), d + 1);
        seen += {xx, yy};
      }
    }
