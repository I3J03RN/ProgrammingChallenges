...
int main() {
  ...
  // I removed the first lines of the input
  vector<string> ss {"BGSC", "TMWHJNVG", "MQS", "BSLTWNM", "JZFTVGWP", "CTBGQHS", "TJPBW", "GDCZFTQM", "NSHBPF"};
  vector<string> sss = ss;
  ...
  input([&](auto& in) {
    ...
    F0R (_, a) {
      ss[to].pb(ss[f].back());
      ss[f].pop_back();
      s += sss[f].back(); sss[f].pop_back();
    }
    reverse(ALL(s));
    sss[to] += s;
  });
  for (auto s : ss) cout << s.back();;
  cout << endl;
  for (auto s : sss) cout << s.back();;
  cout << endl;
