...
int main() {
  ...
  set<ii> free;
  input([&](auto& in) {
    int x, y, xx, yy; string s; char c;
    in >> s >> s >> c >> c >> x >> c >> c >> c >> y >> s >> s >> s >> s >> s >> c >> c >> xx >> c >> c >> c >> yy;
    ss.eb(x, y);
    bs.eb(xx, yy);
    int d = abs(x - xx) + abs(y - yy);
    constexpr int yd = 2000000;
    if (abs(y - yd) <= d) {
      F0R (i, d - abs(y - yd) + 1) {
        free += {x + i, yd};
        free += {x - i, yd};
  ...
  F0R (X, mx + 1) {
    ...
    F0R (i, SZ(ss)) {
      ...
      int d = abs(x - xx) + abs(y - yy);
      if (abs(x - X) <= d) {
        int l = y - (d - abs(x - X)), r = y + (d - abs(x - X));
        set<ii> ne;
        for (auto [a, b] : pos) {
          if (a < l) ne += {a, min(l - 1, b)};
          if (r < b) ne += {max(r + 1, a), b};
        }
        pos.swap(ne);
      }
    }
