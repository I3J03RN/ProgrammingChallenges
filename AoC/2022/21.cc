...
int main() {
  ...
  map<string, ll> val;
  map<string, tuple<string, string, string>> op;
  input([&](auto& in) {
    string id, a, o, b;
    in >> id >> a;
    id.pop_back();
  ...
  auto orig = val;
  while (not val.count("root")) {
    for (auto [id, ab] : op) {
      auto& [a, o, b] = ab;
      if (val.count(id)) continue;
      if (val.count(a) and val.count(b)) {
        if (o == "+") val[id] = val[a] + val[b];
        if (o == "*") val[id] = val[a] * val[b];
        if (o == "-") val[id] = val[a] - val[b];
        if (o == "/") val[id] = val[a] / val[b];
      }
  ...
  val.swap(orig);
  val.erase("humn");
  while (not val.count("humn")) {
    for (auto [id, ab] : op) {
      auto& [a, o, b] = ab;
      if (id == "root") {
        if (val.count(a) + val.count(b) == 1) {
          if (val.count(a)) val[b] = val[a];
          else val[a] = val[b];
        }
      } else {
        if (val.count(id) and val.count(a) and val.count(b)) continue;
        if (val.count(a) and val.count(b)) {
          if (o == "+") val[id] = val[a] + val[b];
          if (o == "*") val[id] = val[a] * val[b];
          if (o == "-") val[id] = val[a] - val[b];
        ...
        if (val.count(id) and val.count(b)) {
          if (o == "+") val[a] = val[id] - val[b];
          if (o == "*") val[a] = val[id] / val[b];
          if (o == "-") val[a] = val[id] + val[b];
          if (o == "/") val[a] = val[id] * val[b];
        }
        if (val.count(a) and val.count(id)) {
          if (o == "+") val[b] = val[id] - val[a];
          if (o == "*") val[b] = val[id] / val[a];
          if (o == "-") val[b] = val[a] - val[id];
          if (o == "/") val[b] = val[a] / val[id];
        }
      }
    }
