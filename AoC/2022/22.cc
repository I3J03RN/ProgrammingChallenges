...
int main() {
  ...
  vi mnr(R), mxr(R);
  F0R (r, R) {
    mnr[r] = gd[r].find_first_not_of(" ");
    mxr[r] = SZ(gd[r]) - 1;
  }
  const int C = *max_element(ALL(mxr)) + 1;
  vi mnc(C, -1), mxc(C, -1);
  F0R (c, C) {
    F0R (r, R) {
      if (safe_gd(r, c) != ' ') {
  ...
  const vi xr{0, 1, 0, -1}, xc{1, 0, -1, 0};
  int dir = 0, r = 0, c = mnr[0];
  for (int i = 0; i < SZ(ins);) {
    if (ins[i] == 'L') {
      dir = (--dir + 4) % 4;
      ++i;
    } else if (ins[i] == 'R') {
      ...
      auto jj = ins.find_first_of("LR", i);
      int j = jj == string::npos ? SZ(ins) : jj;
      int amt = stoi(ins.substr(i, j - i));
      i = j;
      F0R (_, amt) {
        int rr = r + xr[dir], cc = c + xc[dir];
        if (dir % 2) {
          if (rr < mnc[c]) rr = mxc[c];
          if (rr > mxc[c]) rr = mnc[c];
        } else {
          if (cc < mnr[r]) cc = mxr[r];
          if (cc > mxr[r]) cc = mnr[r];
        }
        if (gd[rr][cc] == '.') {
          r = rr;
          c = cc;
  ...
  cout << (r + 1) * 1000 + 4 * (c + 1) + dir << endl;

  // I wrote these transitions out by hand, so this doesn't work for the sample
  if (R < 50) return 0;
  ...
  for (int i = 0; i < SZ(ins);) {
    if (ins[i] == 'L') {
      ...
      F0R (_, amt) {
        int rr = r + xr[dir], cc = c + xc[dir];
        int ndir = dir; // 0 right, 1 down, 2 left, 3 up
        if (dir % 2) {
          if (rr < mnc[c]) {
            if (c < 50) {
              ndir = 0;
        ...
        dout << dvary(rr, cc, ndir, gd[rr][cc], r, c, dir) << endl;
      }
      dout << dvar("----------------") << endl;
      dout << pp(gd2, "\n") << endl;
      dout << dvar("----------------") << endl;
