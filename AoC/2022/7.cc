...
int main() {
  ...
  int idx = 1;
  dir d = y_combinator([&](auto self) -> dir {
    dir d;
    while (idx < SZ(in)) {
      string c = in[idx];
      idx++;
  ...
  auto calc_size = y_combinator([&](auto self, const dir& d, function<void(ll)> f) -> ll {
    ll sz = 0;
    for (auto [s, ds] : d.ds) sz += self(ds, f);
    for (auto [s, i] : d.fs) sz += i;
    f(sz);
    return sz;
  });

  ll sm = 0;
  ll dsz = calc_size(d, [&](ll sz) { if (sz <= 100000) sm += sz; });

  constexpr ll max_space = 40000000;
  ll best = dsz;
  calc_size(d, [&](ll sz) { if (dsz - sz <= max_space) ckmin(best, sz); });

  cout << sm << endl << best << endl;
