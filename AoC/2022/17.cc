...
template<typename T>
bool operator>(const set<T>& s, const T& e) {
  return not static_cast<bool>(s.count(e));
}
...
// does not work for test input ._.
...
int main() {
  ...
  vector<string> bl; int cnt = 0;

  auto fine = [&](vector<string>& p, int x, int y) -> bool {
    const int R = SZ(p), C = SZ(p[0]);
    if (x < 0 or x + C > 7 or y < 0) return false;
    F0R (r, R) {
      if (y + r < SZ(bl)) {
        F0R (c, C) {
          if (p[r][c] == '#' and bl[y + r][x + c] == '#') return false;
        }
      }
    }
    return true;
  ...
  auto sim_until = [&](int poff, int joff, function<bool()> pred) -> pair<ii, ii> {
    int jidx = joff, pidx = poff;
    bl.clear(); cnt = 0;
    for (; pred(); pidx = ++pidx % 5, ++cnt) {
      auto& p = ps[pidx];
      int x = 2, y = SZ(bl) + 3;

      for (;;) {
        int xx = in[jidx] == '<' ? x - 1 : x + 1;
        jidx = ++jidx % SZ(in);
        if (fine(p, xx, y)) x = xx;
        if (fine(p, x, y - 1)) {
          --y;
  ...
  do {
    seen += {p, j};
    auto [ch, pj] = sim_until(p, j, [&]() -> bool { return bl.empty() or bl.back() != "#######"; });
    tie(p, j) = pj;
    auto [c, h] = ch;
    cyc += c;
    cych += h;
  ...
  ll off = 0;
  ll offh = 0;
  for (int pp = 0, jj = 0; tie(pp, jj) != tie(p, j);) {
    auto [ch, pj] = sim_until(pp, jj, [&]() -> bool { return bl.empty() or bl.back() != "#######"; });
    tie(pp, jj) = pj;
    auto [c, h] = ch;
    off += c;
    offh += h;
  ...
  auto sim = [&](ll its) -> ll {
    ...
    auto [vs, _] = sim_until(pp, jj, [&]() -> bool { return cnt < its; });
    auto [c, h] = vs;
    assert(c == its);
    return res + h;
  };
