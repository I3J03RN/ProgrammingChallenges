...
int main() {
  ...
  int idx = 0;
  input([&](auto& in) {
    ll a; in >> a; vs.eb(a, idx++);
    orig.pb(a);
  });

  auto mix = [&orig](vector<pair<ll, int>>& vs) -> void {
    F0R (i, SZ(vs)) {
      auto it = find(ALL(vs), mp(orig[i], i));
      ll j = it - vs.begin();
      j += orig[i];
      vs.erase(it);
      j %= SZ(vs);
      if (j < 0) j += SZ(vs);
      vs.insert(vs.begin() + j, mp(orig[i], i));
    }
  };

  auto calc_score = [](vector<pair<ll, int>>& vs) -> ll {
    int j = 0; ll sm = 0;
    F0R (i, SZ(vs)) if (vs[i].fi == 0) j = i;
    for (int i : {1000, 2000, 3000}) sm += vs[(j + i) % SZ(vs)].fi;
    return sm;
  };

  mix(vs);
  cout << calc_score(vs) << endl;

  constexpr ll dkey = 811589153ll;
  F0R (i, SZ(vs)) vs[i] = {orig[i] *= dkey, i};
