...
int main() {
  ...
  auto sign = [](int n) -> int { return (n > 0) - (n < 0); };

  set<ii> rocks;
  input([&](auto& in) {
    int x, y, xx, yy; char c; string s; in >> xx >> c >> yy;
    rocks += {xx, yy};
    for (; in >> s >> x >> c >> y; xx = x, yy = y) {
      for (int i = x, j = y; i != xx or j != yy; i += sign(xx - x), j += sign(yy - y)) rocks += {i, j};
    }
  });

  int mxy = 0;
  for (auto [_, y] : rocks) ckmax(mxy, y);

  int sc = 0;
  for (set<ii> blocked = rocks;; ++sc) {
    int x = 500, y = 0;
    bool placed = false;
    for (; y <= mxy; ++y) {
      if (blocked < mp(x, y + 1)) {
        if (blocked < mp(x - 1, y + 1)) {
          if (blocked < mp(x + 1, y + 1)) {
            blocked += {x, y};
            placed = true;
            break;
  ...
  sc = 0;
  for (set<ii> blocked = rocks; not (blocked < mp(500, 0)); ++sc) {
    int x = 500, y = 0;
    for (; y <= mxy; ++y) {
      if (blocked < mp(x, y + 1)) {
        if (blocked < mp(x - 1, y + 1)) {
