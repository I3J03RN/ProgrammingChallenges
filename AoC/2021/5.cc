...
int main() {
  ...
  map<ii, int> s1, s2;
  input([&](auto& in){
    int x, y, xp, yp; char c; in >> x >> c >> y >> c >> c >> xp >> c >> yp;
    F0R (i, max(abs(xp - x), abs(yp - y)) + 1) {
      ii co = {x + sgn(xp - x) * i, y + sgn(yp - y) * i};
      ++s2[co];
      if (x == xp or y == yp) ++s1[co];
    }
  });
  
  ll sc1 = 0, sc2 = 0;
  for (auto& [_, v] : s1) sc1 += v > 1;
  for (auto& [_, v] : s2) sc2 += v > 1;
  cout << sc1 << endl << sc2 << endl;
