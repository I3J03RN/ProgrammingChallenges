...
array<int, 4> dc = {0, 1, 0, -1}, dr{1, 0, -1, 0};
...
int main() {
  ...
  auto ok = [&](int r, int c) { return 0 <= min(r, c) and r < 5 * R and c < 5 * C; };
  auto lvl = [&](int r, int c) { int add = r / R + c / C - 1; return (m[r % R][c % C] - '0' + add) % 9 + 1; };

  map<ii, ll> dist;
  F0R (r, 5 * R) F0R (c, 5 * C) dist[{r, c}] = numeric_limits<ll>::max() - 1000;
  set<pair<ll, ii>> q;
  q.emplace(0, mp(0, 0));
  dist[{0, 0}] = 0;
  while (!q.empty()) {
    ...
    F0R (i, 4) {
      int r = v.fi + dr[i], c = v.se + dc[i];
      if (ok(r, c) and ckmin(dist[{r, c}], dist[v] + lvl(r, c))) {
        q.emplace(dist[{r, c}], mp(r, c));
      }
    }
  }

  cout << dist[{R - 1, C - 1}] << endl;
  cout << dist[{5 * R - 1, 5 * C - 1}] << endl;
