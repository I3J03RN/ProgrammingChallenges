...
array<function<iii(iii)>, 6> uffs {
  [](iii a) -> iii { auto [x, y, z] = a; return {x, y, z}; },
  [](iii a) -> iii { auto [x, y, z] = a; return {x, z, y}; },
  [](iii a) -> iii { auto [x, y, z] = a; return {y, x, z}; },
  [](iii a) -> iii { auto [x, y, z] = a; return {y, z, x}; },
  [](iii a) -> iii { auto [x, y, z] = a; return {z, x, y}; },
  [](iii a) -> iii { auto [x, y, z] = a; return {z, y, x}; }};

iii trans(iii t, int xm, int ym, int zm, int upidx) {
  return uffs[upidx](t * mt(xm, ym, zm));
}

optional<iii> match(set<iii>& bs, vector<iii>& o) {
  for (int xm : {-1, 1}) {
    for (int ym : {-1, 1}) {
      for (int zm : {-1, 1}) {
        F0R (upixd, SZ(uffs)) {
          for (iii a : bs) {
            F0R (i, SZ(o) - 11) {
              iii r = o[i];
              iii tr = a - trans(r, xm, ym, zm, upixd);
              int cnt = 0;
              for (iii b : o) {
                cnt += count(ALL(bs), trans(b, xm, ym, zm, upixd) + tr);
              }
              // if (upixd == 3) dout << dvar(xm, ym, zm, upixd, tr, cnt) << endl;
              if (cnt >= 12) {
                for (iii b : o) {
                  bs += trans(b, xm, ym, zm, upixd) + tr;
                }
                // cout << dvar(tr) << endl;
                return {trans({0, 0, 0}, xm, ym, zm, upixd) + tr};
              }
            }
          }
        }
...
int main() {
  ...
  set<iii> bm(ALL(bs[0])), sc{{0, 0, 0}};
  vector<bool> matched(SZ(bs));
  matched[0] = true;
  while (count(ALL(matched), false)) {
    FOR (i, 1, SZ(bs)) {
      if (not matched[i]) {
        if (auto res = match(bm, bs[i]); res) {
          matched[i] = true;
          sc += res.value();
          cout << "matched " << i << endl;
          break;
  ...
  cout << SZ(bm) << endl;
  int mx = 0;
  for (iii a : sc) for (iii b : sc) ckmax(mx, md(a, b));
  cout << mx << endl;
