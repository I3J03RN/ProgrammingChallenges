...
int main() {
  ...
  auto solve = [&print](vector<string> m) -> ll {
    ...
    int bs = SZ(m) - 3;
    vi init{bs, bs, bs, bs, -1, -1, -1, -1, -1, -1, -1};
    map<vi, int> dist;
    map<vi, vi> par;
    set<pair<int, vi>> q;
    dist[init] = 0;
    ...
    dout << dvar("No path found!") << endl;

    return -1;
  };
  
  cout << solve(m) << endl;
  vector<string> p2{"  #D#C#B#A#", "  #D#B#A#C#"};
