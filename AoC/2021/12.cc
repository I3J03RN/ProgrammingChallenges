...
int main() {
  ...
  input([&](auto& in){
    string s; in >> s;
    int d = find(ALL(s), '-') - s.begin();
    string a = s.substr(0, d), b = s.substr(d+1);
    for (string c : {a, b}) {
      if (not m.count(c)) {
        m[c] = SZ(adj);
        once.pb('a' <= c[0] and c[0] <= 'z');
        adj.pb({});
      }
    }
    adj[m[a]].pb(m[b]);
  ...
  vi seen(SZ(adj));
  auto paths = y_combinator([&](auto self, int v, int s, int g, int d) -> int {
    if (once[v] and seen[v]) {
      if (d == 0 or v == s) return 0;
      --d;
    }
    if (v != g) ++seen[v];
    else return 1;
  ...
  cout << paths(m["start"], m["start"], m["end"], 0) << endl;
  cout << paths(m["start"], m["start"], m["end"], 1) << endl;
