...
int main() {
  ...
  string start;
  map<string, char> m;
  map<char, vector<string>> m2;
  bool f = true;
  input([&](auto& in){
    if (f) {
      if (not (in >> start)) f = false;
    } else {
      string r; char c; in >> r >> c >> c >> c;
      m2[c].pb(r);
      m[r] = c;
    }
  });

  auto step = [&](map<string, ll>& a) -> map<string, ll> {
    map<string, ll> out;
    F0R (i, 26 * 26) {
      string cur(2, '?');
      cur[0] = 'A' + i / 26;
      cur[1] = 'A' + i % 26;
      F0R (j, 2) {
        for (string s : m2[cur[j]]) {
          if (s[1 - j] == cur[1 - j]) out[cur] += a[s];
        }
      }
    }
    return out;
  ...
  auto calc = [&](string st, int its) -> ll {
    map<string, ll> cnt;
    F0R (i, SZ(st) - 1) ++cnt[st.substr(i, 2)];
    F0R (_, its) cnt = step(cnt);
    vector<ll> cnt2(26);
    for (auto [s, v] : cnt) {
      F0R (i, 2) cnt2[s[i] - 'A'] += v;
    }
    ll mn = numeric_limits<ll>::max(), mx = numeric_limits<ll>::min();
    --cnt2[st.front() - 'A']; --cnt2[st.back() - 'A'];
    for (auto& i : cnt2) i /= 2;
    ++cnt2[st.front() - 'A']; ++cnt2[st.back() - 'A'];
    for (ll i : cnt2) if (i) {
      ckmin(mn, i); ckmax(mx, i);
    }
    return mx - mn;
  };

  cout << calc(start, 10) << endl << calc(start, 40) << endl;
