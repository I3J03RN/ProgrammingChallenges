...
int main() {
  ...
  input([&](auto& in){
    ...
    s = s.substr(2);
    s.pop_back();
    xmn = stoi(s.substr(0, s.find('.')));
    xmx = stoi(s.substr(s.find('.') + 2));
    in >> s;
    s = s.substr(2);
    ymn = stoi(s.substr(0, s.find('.')));
    ymx = stoi(s.substr(s.find('.') + 2));
  });


  ll best = 0; int cnt = 0;
  FOR (x, 1, xmx + 1) { 
    for (int y = ymn; y <= -ymn; ++y) {
      for (int xx = 0, yy = 0, xv = x, yv = y; yy >= ymn; yy += yv--, xx += xv--, ckmax(xv, 0)) {
        if (xmn <= xx and xx <= xmx and ymn <= yy and yy <= ymx) {
          ckmax(best, gauss(max(y, 0)));
          ++cnt;
          break;
        }
      }
