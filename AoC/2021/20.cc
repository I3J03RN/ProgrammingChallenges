...
int main() {
  ...
  vector<string> mm = input([&](auto&){});
  string enc = mm[0];
  vector<string> m(2 + ALL(mm));

  auto get = [](vector<string>& st, int x, int y, char o) -> char {
    if (min(x, y) < 0 or x >= SZ(st) or y >= SZ(st[0])) return o;
    return st[x][y];
  };

  auto conv = [](string s) -> int {
    int v = 0;
    for (char c : s) v = v * 2 + (c == '#');
    return v;
  };

  int inc = 2;
  auto step = [&](vector<string> st, char o) -> vector<string> {
    vector<string> out(SZ(st) + inc * 2, string(SZ(st[0]) + inc * 2, '?'));
    F0R (x, SZ(out)) {
      F0R (y, SZ(out[0])) {
        string s;
        for (int xd : {-1, 0, 1}) for (int yd : {-1, 0, 1}) 
          s += get(st, x + xd - inc, y + yd - inc, o);
        out[x][y] = enc[conv(s)];
      }
    }
    return out;
  ...
  string os = enc[0] == '#' ? ".#" : "..";
  F0R (_, 2) m = step(m, os[_]);
  ll sc = 0;
  for (auto& v : m) sc += count(ALL(v), '#');
  cout << sc << endl;
  F0R (_, 48) m = step(m, os[_ & 1]);
  sc = 0;
  for (auto& v : m) sc += count(ALL(v), '#');
  cout << sc << endl;
