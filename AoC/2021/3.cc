...
int cnt(const vector<string>& ss, int i) {
  int sc = 0;
  F0R (j, SZ(ss)) sc += ss[j][i] == '1';
  return sc;
}
...
int main() {
  ...
  ll sc = 0;
  F0R (i, n) {
    sc += (2 * cnt(ss, n - 1 - i) > SZ(ss)) << i; 
  }
  cout << sc * (((1 << n) - 1) & ~sc) << endl;

  vector<string> as = ss, bs;
  F0R (i, n) {
    if (SZ(as) == 1) break;
    bs.clear();
    char ok = '0' + (2 * cnt(as, i) >= SZ(as));
    F0R (j, SZ(as)) {
      if (ok == as[j][i]) bs.pb(as[j]);
    }
    as.swap(bs);
  }
  ll sc2 = conv(as[0]);
