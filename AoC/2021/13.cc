...
int main() {
  ...
  input([&](auto& in){
    if (f) {
      int a, b; char c; if (in >> a >> c >> b) ps.emplace(a, b); else f = false;
    } else {
      string s; in >> s >> s >> s;
      fs.eb('x' == s[0], stoi(s.substr(2)));
    }
  });

  auto step = [](const set<ii>& s, ii fold) -> set<ii> {
    set<ii> o;
    for (auto p : s) {
      if (not fold.fi) swap(p.fi, p.se);
      if (p.fi > fold.se) p.fi = 2 * fold.se - p.fi;
      if (not fold.fi) swap(p.fi, p.se);
      o += p;
    }
    return o;
  ...
  cout << SZ(step(ps, fs[0])) << endl;
  for (auto p : fs) ps = step(ps, p);
  int xmn = 0, xmx = 0, ymn = 0, ymx = 0; 
  for (auto [x, y] : ps) {
    ckmax(xmx, x); ckmax(ymx, y);
    ckmin(xmn, x); ckmin(ymn, y);
  }
  vector<string> mes(ymx - ymn + 1, string(xmx - xmn + 1, '.'));
  for (auto [x, y] : ps) {
    mes[y - ymn][x - xmn] = '#';
  }
  cout << pp(mes, "\n", "") << endl;
