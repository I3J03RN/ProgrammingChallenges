...
array<int, 8> xd{1, 1, 1, 0, -1, -1, -1, 0}, yd{1, 0, -1, -1, -1, 0, 1, 1};
...
int main() {
  ...
  auto ok = [](int i, int j) {
    return 0 <= min(i, j) and max(i, j) < 10;
  };

  auto step = [&](vvi vs) -> pair<vvi, int> {
    queue<ii> q;
    F0R (i, 10) {
      F0R (j, 10) {
        if (++vs[i][j] == 10) {
