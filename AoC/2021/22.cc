...
bool overlap(const cube& a, const cube& b) {
  F0R (_, 3) {
    if (a.mx[_] < b.mn[_] or a.mn[_] > b.mx[_]) return false;
  }
  return true;
}

// split up cube b
vector<cube> intersect(const cube& a, cube b) {
  vector<cube> out;
  if (not overlap(a, b)) return {b};
  F0R (i, 3) {
    if (b.mx[i] > a.mx[i]) {
      cube bp = b;
      bp.mn[i] = a.mx[i] + 1;
...
ll vol(const vector<cube>& cs) {
  ll sc = 0;
  for (const auto& c : cs) {
    ll v = 1;
    F0R (i, 3) v *= c.mx[i] - c.mn[i] + 1;
    sc += v;
  }
  return sc;
}
...
int main() {
  ...
  vector<cmd> cmds;
  input([&](auto& in){
    string s; array<int, 3> mn, mx; char c;
    F0R (i, 3) {
      if (i) in >> c; else in >> s;
      in >> c >> c >> mn[i] >> c >> c >> mx[i];
    }
    cmds.pb({s == "on", {mn, mx}});
  });

  vector<cube> as, bs;
  auto go = [&](bool filter) -> void {
    as.clear();
    for (auto& c : cmds) {
      bs.clear();
      cube cur{c.c};
      if (filter and (*min_element(ALL(cur.mn)) < -50 or 50 < *max_element(ALL(cur.mx)))) continue;
      for (const auto& a : as) {
        auto res = intersect(cur, a);
        bs.insert(bs.end(), ALL(res));
      }
      if (c.type) bs.pb(cur);
      as.swap(bs);
    }
