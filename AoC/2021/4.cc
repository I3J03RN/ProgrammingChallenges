...
ll win(const vvi& b, const vi& ns) {
  bool ok = true;
  F0R (r, 5) {
    ok = true;
    F0R (c, 5) {
      ok = ok && (find(ALL(ns), b[r][c]) != ns.end());
    }
    if (ok) goto good;
    ok = true;
    F0R (c, 5) {
      ok = ok && (find(ALL(ns), b[c][r]) != ns.end());
    }
    if (ok) goto good;
  }
  ok = true;
  F0R (c, 5) {
    ok = ok && (find(ALL(ns), b[c][c]) != ns.end());
  }
  if (ok) goto good;
  ok = true;
  F0R (c, 5) {
    ok = ok && (find(ALL(ns), b[c][5 - 1 - c]) != ns.end());
  }
  if (ok) goto good;
  return -1;
 good:
  ...
  F0R (r, 5) {
    F0R (c, 5) {
      if (find(ALL(ns), b[r][c]) == ns.end()) sc += b[r][c];
    }
  }
  return sc * ns.back();
}
...
int main() {
  ...
  // input file is modified to contain no commas and empty lines
  ...
  vi ds;
  vector<bool> done(SZ(bs), false);
  vector<ll> scs;
  for (int n : ns) {
    ds.pb(n);
    F0R (i, SZ(bs)) if (not done[i]) {
      if (ll sc = win(bs[i], ds); sc != -1) {
        scs.pb(sc); 
        done[i] = true;
      }
    }
  }
  cout << scs.front() << endl << scs.back() << endl;
