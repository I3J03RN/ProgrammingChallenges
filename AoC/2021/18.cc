...
struct sfp {
  ...
  vector<sfp> cs;
  sfp(ll v) : val{v}, cs{} {}
  sfp(sfp a, sfp b) : val{-1}, cs{a, b} {}
};

ll readval(string& s, int& i) {
  ll val = 0;
  for (; isdigit(s[i]); ++i) {
    val = 10 * val + s[i] - '0';
  }
  // dout << dvarc(i, s[i]) << endl;
  return val;
}

sfp construct(string& s, int& i) {
  // dout << dvar(i, s[i]) << endl;
  if (isdigit(s[i])) {
    return sfp(readval(s, i));
  } else {
    ++i;
    sfp a = construct(s, i);
    ++i;
...
void print(sfp a, int depth = 0) {
  if (SZ(a.cs)) {
    for (auto& s : a.cs) print(s, depth + 2);
  } else {
    cout << string(depth, ' ') << a.val << endl;
  }
}

void addTo(sfp& a, int idx, ll num) {
  if (SZ(a.cs) == 0) {
    a.val += num;
  } else {
    addTo(a.cs[idx], idx, num);
  }
}

tuple<bool, pair<ll, ll>, sfp> explode(sfp a, int lvl = 0) {
  if (SZ(a.cs) == 0) return {false, {0, 0}, a};
  if (lvl == 4) {
    return {true, {a.cs[0].val, a.cs[1].val}, sfp(0)};
  } else {
    auto [b1, p1, l] = explode(a.cs[0], lvl + 1);
    if (b1) {
      addTo(a.cs[1], 0, p1.se);
      return {b1, {p1.fi, 0}, sfp(l, a.cs[1])};
    }
    auto [b2, p2, r] = explode(a.cs[1], lvl + 1);
    if (b2) {
      addTo(a.cs[0], 1, p2.fi);
      return {b2, {0, p2.se}, sfp(a.cs[0], r)};
    }
    return {false, {0, 0}, a};
  }
}

pair<bool, sfp> split(sfp a) {
  if (SZ(a.cs) == 0) {
    if (a.val >= 10) {
      return {true, sfp(a.val / 2, (a.val + 1) / 2)};
    } else {
      return {false, a};
    }
  } else {
    auto [bl, l] = split(a.cs[0]);
    if (bl) {
      return {true, sfp(l, a.cs[1])};
    }
    auto [br, r] = split(a.cs[1]);
    if (br) {
      return {true, sfp(a.cs[0], r)};
    }
    return {false, a};
  }
}
...
ll mag(sfp& a) {
  if (SZ(a.cs)) {
    return 3 * mag(a.cs[0]) + 2 * mag(a.cs[1]);
  } else {
    return a.val;
  }
}
...
int main() {
  ...
  input([&](auto& in){
    string s; in >> s;
    int i = 0;
    sfps.pb(construct(s, i));
    assert(i == SZ(s));
  });

  sfp cur = sfps[0];
  FOR (i, 1, SZ(sfps)) {
    cur = reduce(sfp(cur, sfps[i]));
  }
  cout << mag(cur) << endl;
