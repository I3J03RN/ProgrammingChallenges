...
int main() {
  ...
  int n, k; cin >> n >> k;
  vector<vector<mod>> as(k + 1, vector<mod>(n));
  cin >> as[1];
  fill(ALL(as[0]), 1);

  vector<vector<mod>> ch(k + 1, vector<mod>(k + 1, 1));
  FOR (i, 2, k + 1) {
    FOR (j, 1, i) {
  ...
  vector<vector<mod>> ss = as;
  for (int i = n - 2; ~i; --i) ss[0][i] += ss[0][i + 1];

  FOR (x, 1, k + 1) {
    if (x > 1) F0R (i, n) as[x][i] = as[x - 1][i] * as[1][i];
    if (x > 1) F0R (i, n) ss[x][i] = as[x - 1][i] * as[1][i];
    mod sum = 0;
    for (int i = n - 2; ~i; --i) {
      F0R (j, x + 1) {
        sum += ch[x][j] * as[j][i] * ss[x - j][i + 1];
      }
      ss[x][i] += ss[x][i + 1];
    }
    cout << sum << endl;
