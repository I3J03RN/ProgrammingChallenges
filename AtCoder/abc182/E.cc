...
struct FT {
  ...
  void update(int i, T add) {
    for (--i; i >= 0; i = (i & (i + 1)) - 1) A[i] += add;
  }
  void update(int i, int j, T add) { update(j, add); update(i, -add); }
  T query(int i) {
    T sum = 0;
    for (; i < n; i |= i + 1) sum += A[i];
    return sum;
  }
};
...
int main() {
  ...
  dout << dvar(ro, co) << endl;

  vector<FT<int>> ftro(R, C), ftco(C, R);
  for (auto [r, c] : bs) {
    auto itr = ro[r].lower_bound(c);
    auto itr2 = prev(itr);
    ftro[r].update(*itr2 + 1, *itr, 1);
    auto itc = co[c].lower_bound(r);
    auto itc2 = prev(itc);
    ftco[c].update(*itc2 + 1, *itc, 1);
    dout << dvarr(r, c, *itr2+1, *itr, *itc2+1, *itc) << endl;
  }

  int sum = 0;
  F0R (r, R) {
    F0R (c, C) {
      sum += !!(ftro[r].query(c) + ftco[c].query(r));
    }
  }
  cout << sum << endl;
